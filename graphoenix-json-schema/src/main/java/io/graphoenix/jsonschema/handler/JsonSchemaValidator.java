...
public class JsonSchemaValidator implements OperationBeforeHandler {
  ...
  public JsonSchemaValidator(
      ...
      JsonSchemaResourceURNFactory jsonSchemaResourceURNFactory,
      Instance<Format> formatInstance) {
  ...
  public Set<ValidationMessage> validate(String jsonSchemaName, String json)
      throws JsonProcessingException {
    return factory
        .getSchema(jsonSchemaManager.getJsonSchema(jsonSchemaName))
                ...
                .map(
                    validationMessage ->
                        new GraphQLError(validationMessage.getMessage())
                            .setPath(validationMessageToGraphQLPath(validationMessage))))
        .collectList()
        .flatMap(
            graphQLErrors -> {
  ...
  protected Stream<ValidationMessage> validateSelection(Operation operation, Field field) {
    try {
      ...
      return validate(
          operationTypeName + "_" + field.getName() + "_" + SUFFIX_ARGUMENTS,
          field.getArguments().toJson())
          .stream();
  ...
  private String validationMessageToGraphQLPath(ValidationMessage validationMessage) {
    Stream<String> pathStream =
        Stream.of(validationMessage.getPath().split("\\."))
            .flatMap(
                item -> {
                  if (item.contains("[")) {
                    return Stream.of(
    ...
    if (validationMessage.getType().equals("required")) {
      return Stream.concat(pathStream, Stream.of(validationMessage.getArguments()[0]))
          .collect(Collectors.joining("/"));

    } else {
      return pathStream.collect(Collectors.joining("/"));
    }
  }
}
