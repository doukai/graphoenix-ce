...
public class GossipPackageCluster implements Runner {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(GossipPackageCluster.class.getName());
  ...
  @Inject
  public GossipPackageCluster(
      GossipConfig gossipConfig,
      PackageManager packageManager,
      GossipPackageProvider gossipPackageProvider) {
  ...
  public void run() {
    new ClusterImpl()
        ...
        .config(opts -> opts.metadata(Map.of(URLS_NAME, getURLs())))
                ...
                new ClusterMessageHandler() {
                  ...
                  public void onMembershipEvent(MembershipEvent event) {
                    switch (event.type()) {
                      ...
                      case UPDATED:
                        cluster
                            .metadata(event.member())
                            .ifPresent(
                                metadata ->
                                    gossipPackageProvider.mergeMemberURLs(
                                        event.member(),
                                        (List<Map<String, Object>>)
                                            ((Map<String, ?>) metadata).get(URLS_NAME)));
                      ...
                      case LEAVING:
                        cluster
                            ...
                            .ifPresent(
                                metadata ->
                                    gossipPackageProvider.removeMemberURLs(
                                        event.member(),
                                        (List<Map<String, Object>>)
                                            ((Map<String, ?>) metadata).get(URLS_NAME)));
