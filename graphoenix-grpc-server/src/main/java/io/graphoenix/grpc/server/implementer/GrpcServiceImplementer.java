...
public class GrpcServiceImplementer {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(GrpcServiceImplementer.class.getName());
  ...
  public void writeToFiler(Filer filer) throws IOException {
        ...
        .map(
            fieldDefinition ->
                new AbstractMap.SimpleEntry<>(
                    fieldDefinition.getPackageName().orElseGet(packageConfig::getPackageName),
                    fieldDefinition))
        .collect(
            Collectors.groupingBy(
                Map.Entry<String, FieldDefinition>::getKey,
                Collectors.mapping(
                    Map.Entry<String, FieldDefinition>::getValue, Collectors.toList())))
        .forEach(
            (packageName, fieldDefinitionList) -> {
              try {
                this.buildTypeServiceImplClass(packageName, TYPE_QUERY_NAME, fieldDefinitionList)
                    .writeTo(filer);
    ...
    documentManager.getDocument().getMutationOperationType().stream()
        ...
        .forEach(
            (packageName, fieldDefinitionList) -> {
              try {
                this.buildTypeServiceImplClass(packageName, TYPE_MUTATION_NAME, fieldDefinitionList)
                    .writeTo(filer);
                logger.info(
                    "{}.Grpc" + TYPE_MUTATION_NAME + "ServiceImpl build success", packageName);
              } catch (IOException e) {
                logger.error(e.getMessage(), e);
              }
            });
  ...
  private JavaFile buildTypeServiceImplClass(
      String packageName, String operationTypeName, List<FieldDefinition> fieldDefinitionList) {
    TypeSpec typeSpec =
        buildOperationTypeServiceImpl(packageName, operationTypeName, fieldDefinitionList);
    return JavaFile.builder(packageName + ".grpc", typeSpec).build();
  }

  private TypeSpec buildOperationTypeServiceImpl(
      String packageName, String operationTypeName, List<FieldDefinition> fieldDefinitionList) {
  ...
  private MethodSpec buildConstructor() {
    return MethodSpec.constructorBuilder()
        .addModifiers(Modifier.PUBLIC)
        .addStatement(
            "this.documentManager = $T.current().select($T.class).get()",
            ClassName.get(CDI.class),
            ClassName.get(DocumentManager.class))
        .addStatement(
            "this.operationHandler = $T.current().select($T.class).get()",
            ClassName.get(CDI.class),
            ClassName.get(OperationHandler.class))
        .addStatement(
            "this.jsonProvider = $T.current().select($T.class).get()",
            ClassName.get(CDI.class),
            ClassName.get(JsonProvider.class))
        .addStatement(
            "this.protobufConverter = $T.current().select($T.class).get()",
            ClassName.get(CDI.class),
            ClassName.get(ProtobufConverter.class))
        .build();
  }
  ...
  private MethodSpec buildTypeMethod(
      String packageName, String operationTypeName, FieldDefinition fieldDefinition) {
    String grpcPackageName = packageName + ".grpc";
    ...
    ParameterizedTypeName responseClassName =
    ...
    return MethodSpec.methodBuilder(grpcHandlerMethodName)
        ...
        .addStatement(
            "$T fieldDefinition = documentManager.getDocument().getObjectTypeOrError($S).getField($S)",
                        ...
                        ? CodeBlock.of(
                            ...
                            fieldDefinition.isConnectionField()
                                ? documentManager
                                    .getFieldTypeDefinition(
                                        documentManager
                                            .getFieldTypeDefinition(
                                                fieldTypeDefinition
                                                    .asObject()
                                                    .getFieldOrError(FIELD_EDGES_NAME))
                                            .asObject()
                                    ...
                                    .stream()
                                    .filter(
                                        subFieldDefinition -> !subFieldDefinition.isFunctionField())
                                    .filter(
                                        subFieldDefinition ->
                                            documentManager
                                    ...
                                    .collect(
                                        Collectors.joining(" ", "{totalCount edges {node {", "}}}"))
  ...
  private TypeSpec buildGraphQLServiceImpl(String packageName) {
    return TypeSpec.classBuilder("GrpcGraphQLServiceImpl")
        .superclass(
            ClassName.get(packageName + ".grpc", "GraphQLServiceGrpc", "GraphQLServiceImplBase"))
        .addModifiers(Modifier.PUBLIC)
        .addField(
            FieldSpec.builder(
        ...
        .addMethod(
            MethodSpec.constructorBuilder()
                .addModifiers(Modifier.PUBLIC)
                .addStatement(
                    "this.fetchHandler = $T.current().select($T.class).get()",
                    ClassName.get(CDI.class),
                    ClassName.get(FetchHandler.class))
                .build())
  ...
  private MethodSpec buildOperationMethod(String packageName) {
    ...
    return MethodSpec.methodBuilder("request")
        ...
        .addStatement(
            CodeBlock.builder()
                .add(
                    "fetchHandler.request($T.fromString($L.getRequest()))\n",
                    ClassName.get(Operation.class),
                    requestParameterName)
                .indent()
                .add(
                    ".map(jsonValue -> $T.newBuilder().setResponse(jsonValue.toString()).build())\n",
                    ClassName.get(grpcPackageName, "GraphQLResponse"))
                .add(
                    ".subscribe(responseObserver::onNext, responseObserver::onError, responseObserver::onCompleted)")
                .unindent()
