...
public class ProtobufConverter {
  ...
  public ProtobufConverter(
      DocumentManager documentManager, JsonProvider jsonProvider, Jsonb jsonb) {
  ...
  public JsonValue toJsonValue(MessageOrBuilder messageOrBuilder, FieldDefinition fieldDefinition) {
    try {
      return toGraphQLJsonValue(
          jsonProvider
              .createReader(new StringReader(JsonFormat.printer().print(messageOrBuilder)))
              .readValue(),
          fieldDefinition);
  ...
  public Message fromJsonValue(
      JsonValue jsonValue, Message.Builder builder, FieldDefinition fieldDefinition) {
    try {
      JsonFormat.parser()
        ...
        .map(
            entry ->
                new AbstractMap.SimpleEntry<>(
                    entry.getKey(),
                    toGraphQLJsonValue(
                        entry.getValue(), fieldDefinition.getArgument(entry.getKey()))))
            ...
            String enumSuffix =
                "_"
                    + CaseFormat.UPPER_CAMEL.to(
                        CaseFormat.UPPER_UNDERSCORE, inputValueTypeDefinition.getName());
            String enumValue = ((JsonString) jsonValue).getString();
            return jsonProvider.createValue(
                enumValue.substring(0, enumValue.lastIndexOf(enumSuffix)));
        ...
        default:
          if (fieldTypeDefinition.isEnum()) {
            String enumSuffix =
                "_"
                    + CaseFormat.UPPER_CAMEL.to(
                        CaseFormat.UPPER_UNDERSCORE, fieldTypeDefinition.getName());
            String enumValue = ((JsonString) jsonValue).getString();
            ...
            return jsonProvider.createValue(
                JsonFormat.printer()
                    .print(
                        Decimal.newBuilder()
                            .setValueBytes(
                                ByteString.copyFrom(bigDecimal.unscaledValue().toByteArray()))
                            .build()));
