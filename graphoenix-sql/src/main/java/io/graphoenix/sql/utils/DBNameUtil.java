...
public final class DBNameUtil {
  ...
  public static String graphqlTypeNameToTableName(String graphqlTypeName) {
    if (graphqlTypeName.startsWith(PREFIX_INTROSPECTION)) {
      return nameToDBEscape(
          PREFIX_INTROSPECTION
              + CaseFormat.UPPER_CAMEL.to(
                  CaseFormat.LOWER_UNDERSCORE,
                  graphqlTypeName.replaceFirst(PREFIX_INTROSPECTION, "")));
    }
  ...
  public static String graphqlTypeNameToTableAliaName(String graphqlTypeName, int level) {
    if (graphqlTypeName.startsWith(PREFIX_INTROSPECTION)) {
      return PREFIX_INTROSPECTION
          + CaseFormat.UPPER_CAMEL.to(
              CaseFormat.LOWER_UNDERSCORE, graphqlTypeName.replaceFirst(PREFIX_INTROSPECTION, ""))
          + "_"
          + level;
    }
  ...
  public static String graphqlFieldNameToVariableName(
      String graphqlTypeName, String graphqlFieldName) {
    return String.join(
        "_",
        CaseFormat.LOWER_CAMEL.to(CaseFormat.LOWER_UNDERSCORE, graphqlTypeName),
        CaseFormat.LOWER_CAMEL.to(CaseFormat.LOWER_UNDERSCORE, graphqlFieldName));
  }

  public static String graphqlDescriptionToDBComment(String description) {
    return stringValueToDBVarchar(
        CharMatcher.anyOf("\"").or(CharMatcher.anyOf("\"\"\"")).trimFrom(description));
  }
