...
public class MutationTranslator {
  ...
  public Stream<Statement> operationToStatementStream(Operation operation) {
    ...
    return operation.getFields().stream()
        ...
        .flatMap(
            field ->
                fieldToMutationStatementStream(
                    operationType, operationType.getFieldOrError(field.getName()), field));
  }

  protected Stream<Statement> fieldToMutationStatementStream(
      ...
      return fieldDefinition
          ...
          .flatMap(
              inputValue -> {
                ...
                List<FieldDefinition> leafFieldDefinitionList =
                                ...
                                (Statement)
                                    insertSelectExpression(
                                        ...
                                        leafFieldDefinitionList.stream()
                                            .map(
                                                subField ->
                                                    graphqlFieldToColumn(table, subField.getName()))
                                            .collect(Collectors.toList()),
                                        selectVariablesFromJsonObjectArray(
                            ...
                            List<FieldDefinition> leafFieldDefinitionList =
                                fieldTypeDefinition.asObject().getFields().stream()
                                    ...
                                    .filter(
                                        subField ->
                                            !documentManager
                                                .getFieldTypeDefinition(subField)
                                                .isObject())
                                    .collect(Collectors.toList());
                                                        ...
                                                        .map(
                                                            subField ->
                                                                graphqlFieldToColumn(
                                                                    table, subField.getName()))
                                                        .collect(Collectors.toList()),
                                                    selectVariablesFromJsonObject(
                                                        leafFieldDefinitionList,
                                                        inputValue,
                                                        valueWithVariable.asVariable())));
                                      } else {
                                                ...
                                                .flatMap(
                                                    arguments ->
                                                        arguments.getArgumentOrEmpty(
                                                            argumentInput.getName()))
                                                .or(
                                                    () ->
                                                        Optional.ofNullable(
                                                            argumentInput.getDefaultValue()))
                                                .stream()
                                                .map(
                                                    valueWithVariable ->
                                                        new AbstractMap.SimpleEntry<>(
                                                            argumentInput, valueWithVariable)))
                                    .collect(
                                        Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

                            return inputValueMapToMutationStatementStream(
                                objectType,
                                fieldDefinition,
                                inputValueValueWithVariableMap,
                                field.isMerge());
                          }));
    }
  ...
  protected Stream<Statement> objectToMutationStatementStream(
    ...
    Map<InputValue, ValueWithVariable> inputValueValueWithVariableMap =
        Stream.ofNullable(inputValueTypeDefinition.asInputObject().getInputValues())
            .flatMap(Collection::stream)
            .flatMap(
                fieldInput ->
  ...
  protected Stream<Statement> inputValueMapToMutationStatementStream(
      ObjectType objectType,
      FieldDefinition fieldDefinition,
      Map<InputValue, ValueWithVariable> inputValueValueWithVariableMap,
      boolean merge) {
    ...
    return inputValueMapToMutationStatementStream(
        objectType, null, fieldDefinition, inputValueValueWithVariableMap, merge, 0, 0);
  }
  ...
  protected Stream<Statement> inputValueMapToMutationStatementStream(
    ...
    Map<String, ValueWithVariable> leafValueWithVariableMap =
        leafFieldDefinitionList.stream()
            .flatMap(
                subField ->
    ...
    Optional<Map.Entry<InputValue, ValueWithVariable>> whereInputValueEntry =
        inputValueValueWithVariableMap.entrySet().stream()
            ...
            .or(
                () ->
                    whereInputValueEntry
                        .flatMap(
                            entry ->
                                entry.getValue().asObject().getValueWithVariableOrEmpty(idName))
                        .filter(ValueWithVariable::isObject)
                        .flatMap(
                            valueWithVariable ->
                                valueWithVariable
            ...
            .orElseGet(
                () ->
                    createInsertIdUserVariable(
                        fieldTypeDefinition.asObject().getName(), idName, level, index));

    Stream<Statement> leafFieldMutationStatement =
        Stream.of(
            whereInputValueEntry
                .flatMap(
                    entry ->
                        argumentsTranslator.inputValueToWhereExpression(
                            objectType, fieldDefinition, entry.getKey(), entry.getValue(), level))
                .map(
                    whereExpression -> {
      ...
      objectFieldMergeMapStatementStream =
          Stream.of(
              mergeObjectMapStatement(
                  objectType, fieldDefinition, parentIdExpression, idExpression));
    }

    Stream<Statement> objectFieldMutationStatementStream =
        fieldTypeDefinition.asObject().getFields().stream()
            .filter(subField -> !subField.isFetchField())
            .filter(subField -> !subField.getType().hasList())
            .filter(subField -> documentManager.getFieldTypeDefinition(subField).isObject())
    ...
    Stream<Statement> listObjectFieldMutationStatementStream =
        fieldTypeDefinition.asObject().getFields().stream()
            .filter(subField -> !subField.isFetchField())
            .filter(subField -> subField.getType().hasList())
            .filter(subField -> documentManager.getFieldTypeDefinition(subField).isObject())
    ...
    Stream<Statement> listLeafFieldMutationStatementStream =
        fieldTypeDefinition.asObject().getFields().stream()
            .filter(subField -> !subField.isFetchField())
            .filter(subField -> subField.getType().hasList())
            .filter(subField -> !documentManager.getFieldTypeDefinition(subField).isObject())
  ...
  protected Stream<Statement> listObjectToMutationStatementStream(
      ObjectType objectType,
      Expression parentIdExpression,
      FieldDefinition fieldDefinition,
      InputValue inputValue,
          ...
          Stream.of(
              insertSelectExpression(
                  ...
                  leafFieldDefinitionList.stream()
                      ...
                      .collect(Collectors.toList()),
                  selectVariablesFromJsonObjectArray(
                      leafFieldDefinitionList, inputValue, valueWithVariable.asVariable())));
    } else if (valueWithVariable.isArray()) {
      if (merge) {
        List<ValueWithVariable> mergeValueWithVariableList =
        ...
        if (!mergeValueWithVariableList.isEmpty()) {
          mutationStatementStream =
              IntStream.range(0, mergeValueWithVariableList.size())
                  .mapToObj(
                      index ->
                          objectToMutationStatementStream(
                              objectType,
    ...
    if (merge) {
      ...
      if (fieldDefinition.hasMapWith()) {
        List<ValueWithVariable> idValueWithVariableList =
            valueWithVariable.asArray().getValueWithVariables().stream()
                .filter(ValueWithVariable::isObject)
                .map(ValueWithVariable::asObject)
                .flatMap(
                            ...
                            .or(
                                () ->
                                    objectValueWithVariable
                                        ...
                                        .flatMap(
                                            id ->
                                                id.getValueWithVariableOrEmpty(
                                                    INPUT_OPERATOR_INPUT_VALUE_VAL_NAME)))
                            .stream())
                .collect(Collectors.toList());
        ...
        return Stream.concat(removeMapStatementStream, mutationStatementStream);
      } else {
        List<ValueWithVariable> removeIdValueWithVariableList =
            valueWithVariable.asArray().getValueWithVariables().stream()
                .filter(ValueWithVariable::isObject)
                .map(ValueWithVariable::asObject)
                .filter(
        ...
        return Stream.concat(mutationStatementStream, removeMapStatementStream);
      }
    } else {
      if (fieldDefinition.hasMapWith()) {
        Statement removeMapStatement =
            removeMapStatement(objectType, fieldDefinition, parentIdExpression, null);
        return Stream.concat(Stream.of(removeMapStatement), mutationStatementStream);
      } else {
        Definition fieldTypeDefinition = documentManager.getFieldTypeDefinition(fieldDefinition);
        ...
        List<Expression> idValueExpressionList =
            IntStream.range(0, valueWithVariable.asArray().size())
                .filter(
                    index ->
                        valueWithVariable.asArray().getValueWithVariables().get(index).isObject())
                .mapToObj(
                    index ->
                        valueWithVariable
                            .asArray()
                            ...
                            .get(index)
                            .asObject()
                            .getValueWithVariableOrEmpty(idFieldName)
                            .flatMap(DBValueUtil::idValueToDBValue)
                            .orElseGet(
                                () ->
                                    createInsertIdUserVariable(
        ...
        Statement removeMapStatement =
            removeMapStatement(
                objectType, fieldDefinition, parentIdExpression, idValueExpressionList);
        return Stream.concat(mutationStatementStream, Stream.of(removeMapStatement));
      }
    }
  }

  protected Stream<Statement> listLeafToStatementStream(
      ObjectType objectType,
      Expression parentIdExpression,
      FieldDefinition fieldDefinition,
      ValueWithVariable valueWithVariable) {
      ...
      return Stream.concat(
          ...
          valueWithVariable.asArray().getValueWithVariables().stream()
              .map(
                  item ->
                      mergeLeafMapStatement(
                          objectType, fieldDefinition, item, parentIdExpression)));
    }
  }

  protected Statement removeMapStatement(
      ObjectType objectType,
      FieldDefinition fieldDefinition,
      Expression parentIdExpression,
      List<Expression> idValueExpressionList) {
    return removeMapStatement(
        objectType, fieldDefinition, parentIdExpression, idValueExpressionList, true);
  }
  ...
  protected Statement removeMapStatement(
    ...
    Column parentIdColumn =
        graphqlFieldToColumn(parentTable, objectType.getIDFieldOrError().getName());
    ...
    if (fieldDefinition.hasMapWith()) {
      ...
      if (idValueExpressionList != null && !idValueExpressionList.isEmpty()) {
        ...
        if (fieldDefinition
            .getMapToOrError()
            .equals(fieldTypeDefinition.asObject().getIDFieldOrError().getName())) {
          inExpression.setRightExpression(
              new Parenthesis(new ExpressionList<>(idValueExpressionList)));
        } else {
          Table table = typeToTable(fieldTypeDefinition.asObject());
          Column column = graphqlFieldToColumn(table, fieldDefinition.getMapToOrError());
          Column idColumn =
              graphqlFieldToColumn(
                  table, fieldTypeDefinition.asObject().getIDFieldOrError().getName());
          inExpression.setRightExpression(
              selectFieldByIdExpressionList(table, column, idColumn, idValueExpressionList));
        }
        return removeExpression(
            withTable,
            new MultiAndExpression(Arrays.asList(withParentColumnEqualsTo, inExpression)));
      } else {
      ...
      if (documentManager.isMapAnchor(objectType, fieldDefinition)) {
        ...
        Column idColumn =
            graphqlFieldToColumn(
                table, fieldTypeDefinition.asObject().getIDFieldOrError().getName());
        EqualsTo columnEqualsTo =
            new EqualsTo().withLeftExpression(column).withRightExpression(parentColumnExpression);
  ...
  protected Statement mergeObjectMapStatement(
      ObjectType objectType,
      FieldDefinition fieldDefinition,
      Expression parentIdExpression,
      Expression idExpression) {
    ...
    Column column = graphqlFieldToColumn(table, fieldDefinition.getMapToOrError());
    Column idColumn =
        graphqlFieldToColumn(table, fieldTypeDefinition.asObject().getIDFieldOrError().getName());
    ...
    if (fieldDefinition.hasMapWith()) {
      ...
      if (documentManager.isMapAnchor(objectType, fieldDefinition)) {
        ...
        return updateExpression(
            parentTable,
            Collections.singletonList(new UpdateSet(parentColumn, columnExpression)),
            parentIdEqualsTo);
      } else {
        EqualsTo idEqualsTo =
            new EqualsTo().withLeftExpression(idColumn).withRightExpression(idExpression);
        return updateExpression(
            table,
            Collections.singletonList(new UpdateSet(column, parentColumnExpression)),
            idEqualsTo);
      }
    }
  }

  protected Statement mergeLeafMapStatement(
      ObjectType objectType,
      FieldDefinition fieldDefinition,
      ValueWithVariable valueWithVariable,
      Expression parentIdExpression) {
  ...
  protected Insert insertExpression(
      Table table, List<Column> columnList, List<Expression> expressionList) {
    return insertExpression(table, columnList, expressionList, false);
  }

  protected Insert insertExpression(
      Table table, List<Column> columnList, List<Expression> expressionList, boolean useDuplicate) {
    Insert insert =
        new Insert()
            .withTable(table)
            .addColumns(columnList)
            .withSelect(new Values().addExpressions(expressionList));
  ...
  protected Insert insertSelectExpression(Table table, List<Column> columnList, Select select) {
    Insert insert = new Insert().withTable(table).addColumns(columnList).withSelect(select);
  ...
  protected Update updateExpression(Table table, List<UpdateSet> updateSetList, Expression where) {
    return new Update().withTable(table).withUpdateSets(updateSetList).withWhere(where);
  }
  ...
  protected Update removeExpression(Table table, Expression whereExpression) {
    return new Update()
        .withTable(table)
        .addUpdateSet(
            new UpdateSet(graphqlFieldToColumn(table, FIELD_DEPRECATED_NAME), new LongValue(1)))
        .withWhere(whereExpression);
  }

  protected ParenthesedSelect selectFieldByIdExpression(
      Table table, Column selectColumn, Column idColumn, Expression idFieldValueExpression) {
  ...
  protected ParenthesedSelect selectFieldByIdExpressionList(
      Table table,
      Column selectColumn,
      Column idColumn,
            ...
            new PlainSelect()
                ...
                .withWhere(
                    new InExpression()
                        .withLeftExpression(idColumn)
                        .withRightExpression(
                            new Parenthesis(new ExpressionList<>(idFieldValueExpressionList)))));
  }

  protected Select selectVariablesFromJsonObjectArray(
                        ...
                        .map(
                            subField ->
                                new ColumnDefinition()
                                    .withColumnName(
                                        graphqlFieldNameToColumnName(subField.getName()))
                                    .withColDataType(
                                        typeTranslator.createColDataType(subField, true))
                                    .addColumnSpecs("PATH", "'$." + subField.getName() + "'"))
                        .collect(Collectors.toList()))
                .withAlias(new Alias(nameToDBEscape(inputValue.getName()))));
  }

  protected Select selectVariablesFromJsonObject(
      List<FieldDefinition> fieldDefinitionList,
      InputValue inputValue,
      ValueWithVariable valueWithVariable) {
    return new PlainSelect()
            ...
            new JsonTableFunction()
                .withJson(
                    new Function()
                        ...
                        .withParameters(
                            new StringValue("["),
                            new JdbcNamedParameter()
                                .withName(valueWithVariable.asVariable().getName()),
                            new StringValue("]")))
                .withPath(new StringValue("$[*]"))
                .withColumnDefinitions(
                    fieldDefinitionList.stream()
