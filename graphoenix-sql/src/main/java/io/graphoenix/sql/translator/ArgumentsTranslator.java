...
public class ArgumentsTranslator {
  ...
  public Optional<Expression> argumentsToWhereExpression(
    ...
    if (fieldTypeDefinition.isObject()) {
              ...
              .flatMap(
                  argumentInput ->
                      Optional.ofNullable(field.getArguments())
                          .flatMap(
                              arguments -> arguments.getArgumentOrEmpty(argumentInput.getName()))
                          .or(() -> Optional.ofNullable(argumentInput.getDefaultValue()))
                          .stream()
                          .map(
                              valueWithVariable ->
                                  new AbstractMap.SimpleEntry<>(argumentInput, valueWithVariable)))
      ...
      return inputValueValueWithVariableMap.entrySet().stream()
          ...
          ? expressionListToMultipleExpression(
              expressionList, isOr(field.getArguments()), isNot(field.getArguments()))
          : expressionListToMultipleExpression(
                  expressionList, isOr(field.getArguments()), isNot(field.getArguments()))
              .map(
                  expression ->
      ...
      return expressionListToMultipleExpression(
          Stream.concat(
                  fieldDefinition
                      .getArgumentOrEmpty(INPUT_OPERATOR_INPUT_VALUE_OPR_NAME)
                      .flatMap(
                          inputValue ->
                              Optional.ofNullable(field.getArguments())
                                  .map(
                                        ...
                                        if (fieldDefinition.getType().hasList()) {
                                          ...
                                          return arguments
                                              ...
                                              .flatMap(
                                                  val ->
                                                      valToExpression(
                                                          column, opr, val, skipNull(arguments)))
                                              .or(
                                                  () ->
                                                      arguments
                                                          .getArgumentOrEmpty(
  ...
  protected Optional<Expression> inputValueToWhereExpression(
      ObjectType objectType,
      FieldDefinition fieldDefinition,
      InputValue inputValue,
      ValueWithVariable valueWithVariable,
      int level) {
    return inputValueToWhereExpression(
        objectType, fieldDefinition, inputValue, valueWithVariable, level, false);
  }
  ...
  protected Optional<Expression> inputValueToWhereExpression(
    ...
    if (fieldTypeDefinition.isObject()) {
                      ...
                      .flatMap(
                          entry ->
                              Stream.of(entry.getValue())
                                  .filter(ValueWithVariable::isArray)
                                  .flatMap(
                                      field -> field.asArray().getValueWithVariables().stream())
                                  .flatMap(
                                      field ->
                                          inputValueToWhereExpression(
                                              objectType,
                  ...
                  .anyMatch(
                      entry ->
                          entry.getKey().getName().equals(INPUT_VALUE_INCLUDE_DEPRECATED_NAME)
                              ...
                              && entry.getValue().asBoolean().getValue())
          ? expressionListToMultipleExpression(
              expressionList, isOr(valueWithVariable), isNot(valueWithVariable))
          : expressionListToMultipleExpression(
                  expressionList, isOr(valueWithVariable), isNot(valueWithVariable))
              .map(
                  expression ->
      ...
      InputObjectType inputObject =
          documentManager.getInputValueTypeDefinition(inputValue).asInputObject();
      return inputObject
          .getInputValueOrEmpty(INPUT_OPERATOR_INPUT_VALUE_OPR_NAME)
          .flatMap(
              fieldInput ->
                  Optional.ofNullable(valueWithVariable)
                      .filter(ValueWithVariable::isObject)
                      ...
                      .map(
                          objectValueWithVariable ->
                              objectValueWithVariable
                                  .getValueWithVariableOrEmpty(fieldInput.getName())
                                  .filter(ValueWithVariable::isEnum)
                                  .map(opr -> opr.asEnum().getValue())
                                  .orElseGet(
                                      () ->
                            ...
                            if (fieldDefinition.getType().hasList()) {
                              ...
                              return objectValueWithVariable
                                  ...
                                  .flatMap(
                                      val ->
                                          valToExpression(
                                              column, opr, val, skipNull(objectValueWithVariable)))
                                  .or(
                                      () ->
                                          objectValueWithVariable
                                              .getValueWithVariableOrEmpty(
                                              ...
                                              .flatMap(
                                                  arr ->
                                                      arrToExpression(
                                                          column,
                                                          opr,
                                                          inputObject.getInputValue(
                                                              INPUT_OPERATOR_INPUT_VALUE_ARR_NAME),
                                                          arr,
                                                          skipNull(objectValueWithVariable))))
                                  .or(() -> valToExpression(column, opr))
                                  .map(
                                      expression ->
                                          existsExpression(
  ...
  protected Optional<Expression> cursorArgumentsToExpression(
    ...
    if (fieldDefinition.getArguments() != null) {
      ...
      if (fieldTypeDefinition.isObject()) {
        ...
        Column column =
            graphqlFieldToColumn(withType.getName(), withToFieldDefinition.getName(), level);
        return Optional.ofNullable(fieldDefinition.getArgument(INPUT_VALUE_AFTER_NAME))
            .flatMap(
                inputValue ->
                    Optional.ofNullable(field.getArguments())
  ...
  protected Optional<Expression> expressionListToMultipleExpression(
      List<Expression> expressionList, boolean isOr, boolean isNot) {
  ...
  protected PlainSelect selectFromFieldType(
    ...
    if (fieldTypeDefinition.isObject()) {
      ...
      PlainSelect plainSelect =
          new PlainSelect().addSelectItems(new AllColumns()).withFromItem(table);
          ...
          .withWhere(
              new MultiAndExpression(
                  Arrays.asList(
                      new EqualsTo()
                          .withLeftExpression(
                              graphqlFieldToColumn(table, fieldDefinition.getMapWithFromOrError()))
                          .withRightExpression(
                              graphqlFieldToColumn(
                                  parentTable, fieldDefinition.getMapFromOrError())),
                      expression,
  ...
  protected Optional<Expression> valToExpression(
    ...
    switch (opr) {
      case INPUT_OPERATOR_INPUT_VALUE_EQ:
        where = new EqualsTo().withLeftExpression(column).withRightExpression(value);
        break;
      case INPUT_OPERATOR_INPUT_VALUE_NEQ:
        where = new NotEqualsTo().withLeftExpression(column).withRightExpression(value);
        break;
      case INPUT_OPERATOR_INPUT_VALUE_LK:
        where = new LikeExpression().withLeftExpression(column).withRightExpression(value);
        break;
      ...
      case INPUT_OPERATOR_INPUT_VALUE_GT:
        where = new GreaterThan().withLeftExpression(column).withRightExpression(value);
        break;
      case INPUT_OPERATOR_INPUT_VALUE_GTE:
        where = new GreaterThanEquals().withLeftExpression(column).withRightExpression(value);
        break;
      case INPUT_OPERATOR_INPUT_VALUE_LT:
        where = new MinorThan().withLeftExpression(column).withRightExpression(value);
        break;
      case INPUT_OPERATOR_INPUT_VALUE_LTE:
        where = new MinorThanEquals().withLeftExpression(column).withRightExpression(value);
        break;
      case INPUT_OPERATOR_INPUT_VALUE_NIL:
        where = new IsNullExpression().withLeftExpression(column);
        break;
  ...
  protected Optional<Expression> arrToExpression(
    ...
    if (arr.isVariable()) {
      ...
      switch (opr) {
        case INPUT_OPERATOR_INPUT_VALUE_IN:
          where = new InExpression().withLeftExpression(column).withRightExpression(value);
          break;
        case INPUT_OPERATOR_INPUT_VALUE_NIN:
          where =
              new InExpression()
                              ...
                              return new MultiAndExpression(
                                  Arrays.asList(
                                      new GreaterThanEquals()
                                          .withLeftExpression(column)
                                          .withRightExpression(
                                              leafValueToDBValue(valueWithVariables.get(0))),
                                      new MinorThanEquals()
                                          .withLeftExpression(column)
                                          .withRightExpression(
                                              leafValueToDBValue(valueWithVariables.get(1)))));
                            } else {
  ...
  protected Expression skipNullExpression(Expression value, Expression where) {
    return new Function()
        .withName("IF")
        .withParameters(new IsNullExpression().withLeftExpression(value), new LongValue(1), where);
  }

  protected ParenthesedSelect selectValueFromJsonArray(
  ...
  protected ExistsExpression existsExpression(PlainSelect plainSelect) {
    return new ExistsExpression()
        .withRightExpression(new ParenthesedSelect().withSelect(plainSelect));
  }
