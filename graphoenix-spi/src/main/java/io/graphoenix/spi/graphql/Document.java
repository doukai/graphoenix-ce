...
public class Document {
  ...
  public Document addDefinitionsByFileName(String fileName, ClassLoader classLoader)
      throws IOException {
    if (Files.exists(Path.of(fileName))) {
      ...
      try {
        ...
        try (FileSystem fileSystem =
            FileSystems.newFileSystem(Objects.requireNonNull(resource).toURI(), env)) {
  ...
  public Document addDefinitionsByPathName(String pathName, ClassLoader classLoader)
      throws IOException {
  ...
  public Document addDefinitionsByPathName(String pathName, String resourcePath)
      throws IOException {
  ...
  public Document addDefinitions(Collection<? extends Definition> definitions) {
    this.definitionMap.putAll(
        (Map<? extends String, ? extends Definition>)
            definitions.stream()
                .map(this::mergeExtension)
                .collect(
                    Collectors.toMap(
  ...
  public Optional<ObjectType> getObjectType(String name) {
    return Optional.ofNullable(getDefinition(name)).map(Definition::asObject);
  }

  public ObjectType getObjectTypeOrError(String name) {
    return getObjectType(name)
        .orElseThrow(() -> new GraphQLErrors(TYPE_DEFINITION_NOT_EXIST.bind(name)));
  }

  public Optional<InterfaceType> getInterfaceType(String name) {
    return Optional.ofNullable(getDefinition(name)).map(Definition::asInterface);
  }

  public InterfaceType getInterfaceTypeOrError(String name) {
    return getInterfaceType(name)
        .orElseThrow(() -> new GraphQLErrors(TYPE_DEFINITION_NOT_EXIST.bind(name)));
  }

  public Stream<ObjectType> getObjectTypes() {
    return getDefinitions().stream().filter(Definition::isObject).map(Definition::asObject);
  }

  public Stream<InterfaceType> getInterfaceTypes() {
    return getDefinitions().stream().filter(Definition::isInterface).map(Definition::asInterface);
  }
  ...
  public Stream<EnumType> getEnums() {
    return getDefinitions().stream().filter(Definition::isEnum).map(Definition::asEnum);
  }

  public Stream<ScalarType> getScalarTypes() {
    return getDefinitions().stream().filter(Definition::isScalar).map(Definition::asScalar);
  }

  public Optional<ScalarType> getScalarType(String name) {
    return Optional.ofNullable(getDefinition(name)).map(Definition::asScalar);
  }

  public Stream<DirectiveDefinition> getDirectives() {
    return getDefinitions().stream().filter(Definition::isDirective).map(Definition::asDirective);
  }

  public Optional<DirectiveDefinition> getDirective(String name) {
    return Optional.ofNullable(getDefinition(name)).map(Definition::asDirective);
  }
  ...
  public Optional<InputObjectType> getInputObjectType(String name) {
    return Optional.ofNullable(getDefinition(name)).map(Definition::asInputObject);
  }

  public InputObjectType getInputObjectTypeOrError(String name) {
    return getInputObjectType(name)
        .orElseThrow(() -> new GraphQLErrors(INPUT_OBJECT_DEFINITION_NOT_EXIST.bind(name)));
  }

  public Optional<EnumType> getEnumType(String name) {
    return Optional.ofNullable(getDefinition(name)).map(Definition::asEnum);
  }
  ...
  public Optional<FragmentDefinition> getFragmentDefinition(String name) {
    return Optional.ofNullable(getDefinition(name)).map(Definition::asFragmentDefinition);
  }

  public FragmentDefinition getFragmentDefinitionOrError(String name) {
    return getFragmentDefinition(name)
        .orElseThrow(() -> new GraphQLErrors(FRAGMENT_NOT_EXIST.bind(name)));
  }
  ...
  public Stream<Operation> getOperations() {
    return getDefinitions().stream().filter(Definition::isOperation).map(Definition::asOperation);
  }
  ...
  public Optional<ObjectType> getQueryOperationType() {
    return Optional.ofNullable(
            getDefinition(getSchema().map(Schema::getQuery).orElse(TYPE_QUERY_NAME)))
        .map(Definition::asObject);
  }

  public Optional<ObjectType> getMutationOperationType() {
    return Optional.ofNullable(
            getDefinition(getSchema().map(Schema::getMutation).orElse(TYPE_MUTATION_NAME)))
        .map(Definition::asObject);
  }

  public Optional<ObjectType> getSubscriptionOperationType() {
    return Optional.ofNullable(
            getDefinition(getSchema().map(Schema::getSubscription).orElse(TYPE_SUBSCRIPTION_NAME)))
        .map(Definition::asObject);
  }

  public ObjectType getQueryOperationTypeOrError() {
    return getQueryOperationType()
        .orElseThrow(() -> new GraphQLErrors(QUERY_TYPE_DEFINITION_NOT_EXIST));
  }

  public ObjectType getMutationOperationTypeOrError() {
    return getMutationOperationType()
        .orElseThrow(() -> new GraphQLErrors(MUTATION_TYPE_DEFINITION_NOT_EXIST));
  }

  public ObjectType getSubscriptionOperationTypeOrError() {
    return getSubscriptionOperationType()
        .orElseThrow(() -> new GraphQLErrors(SUBSCRIBE_TYPE_DEFINITION_NOT_EXIST));
  }

  public Optional<InterfaceType> getMetaInterface() {
    return Optional.ofNullable(getDefinition(INTERFACE_META_NAME))
        .map(definition -> (InterfaceType) definition);
  }
  ...
  public Definition merge(Definition definition) {
    if (definition.isSchema()) {
      ...
      return getScalarType(definition.getName())
          //                    .filter(scalarType -> scalarType.isExtension() ==
          // definition.isExtension())
