...
public class Arguments extends AbstractMap<String, JsonValue>
    ...
    this.arguments =
        argumentsContext.argument().stream()
            .map(
                argumentContext ->
                    new SimpleEntry<>(
                        argumentContext.name().getText(),
                        ValueWithVariable.of(argumentContext.valueWithVariable())))
            .collect(
                Collectors.toMap(Entry::getKey, Entry::getValue, (x, y) -> y, LinkedHashMap::new));
  }
  ...
  public Arguments(AnnotationMirror arguments) {
    this.arguments =
        arguments.getElementValues().entrySet().stream()
            .collect(
                Collectors.toMap(
  ...
  public Arguments(ExecutableElement executableElement, AnnotationMirror fieldAnnotationMirror) {
    this.arguments =
        fieldAnnotationMirror.getElementValues().entrySet().stream()
            .map(
                entry -> {
                  ...
                  if (fieldName.startsWith("$")) {
                    return new AbstractMap.SimpleEntry<>(
                        ...
                        ValueWithVariable.of(
                            annotationValueToVariable(executableElement, entry.getValue())));
                  } else {
                    return new AbstractMap.SimpleEntry<>(
                        fieldName,
                        ValueWithVariable.of(
  ...
  private Object annotationValueToValue(
      ExecutableElement executableElement, AnnotationValue annotationValue) {
  ...
  private Object annotationValueToVariable(
      ExecutableElement executableElement, AnnotationValue annotationValue) {
    Object value = annotationValue.getValue();
    if (value instanceof Collection<?>) {
      return ((Collection<?>) value)
            ...
            .orElseThrow(
                () ->
                    new RuntimeException(
                        value.toString() + " parameter not exist in " + executableElement));
      }
    }
  }

  private Object annotationMirrorToValueWithVariable(
  ...
  public static <K, V> Arguments of(
