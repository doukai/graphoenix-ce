...
public abstract class AbstractDefinition implements Definition {
  ...
  public AbstractDefinition(
      GraphqlParser.DescriptionContext description,
      GraphqlParser.DirectivesContext directivesContext) {
    this(description);
    if (directivesContext != null) {
      setDirectives(
          directivesContext.directive().stream().map(Directive::new).collect(Collectors.toList()));
    }
  }
  ...
  public AbstractDefinition(
      GraphqlParser.NameContext nameContext, GraphqlParser.DescriptionContext description) {
  ...
  public AbstractDefinition(TypeElement typeElement) {
    ...
    this.addDirective(
        new Directive(DIRECTIVE_CLASS_NAME)
            .addArgument(
                DIRECTIVE_CLASS_ARGUMENT_NAME_NAME, typeElement.getQualifiedName().toString())
  ...
  public AbstractDefinition(ExecutableElement executableElement) {
    this.name =
        executableElement.getParameters().stream()
            ...
            .filter(
                variableElement -> !variableElement.getAnnotation(Source.class).name().isBlank())
            .findFirst()
    ...
    this.directiveMap =
        Stream.concat(
                ...
                Stream.of(abstractDefinitions)
                    .flatMap(item -> Stream.ofNullable(item.getDirectives())))
            .flatMap(Collection::stream)
            .filter(distinctByKey(Directive::getName))
            .collect(
                Collectors.toMap(
                    Directive::getName, directive -> directive, (x, y) -> y, LinkedHashMap::new));
    return (T) this;
  }
  ...
  public Directive getDirective(String name) {
    return Optional.ofNullable(directiveMap)
        .map(stringDirectiveMap -> stringDirectiveMap.get(name))
        .orElse(null);
  }
  ...
  public Collection<Directive> getDirectives() {
    return Optional.ofNullable(directiveMap)
        .map(stringDirectiveMap -> stringDirectiveMap.values())
        .orElse(null);
  }
  ...
  public <T extends AbstractDefinition> T addDirectives(Collection<Directive> directives) {
    ...
    this.directiveMap.putAll(
        (Map<? extends String, ? extends Directive>)
            directives.stream()
                .collect(
                    Collectors.toMap(
                        Directive::getName,
  ...
  public Optional<String> getPackageName() {
    return Optional.ofNullable(directiveMap)
        ...
        .flatMap(
            directive ->
                Optional.ofNullable(directive.getArgument(DIRECTIVE_PACKAGE_ARGUMENT_NAME_NAME)))
  ...
  public Optional<String> getClassName() {
    return Optional.ofNullable(directiveMap)
        ...
        .flatMap(
            directive ->
                Optional.ofNullable(directive.getArgument(DIRECTIVE_CLASS_ARGUMENT_NAME_NAME)))
        .filter(ValueWithVariable::isString)
        .map(valueWithVariable -> (StringValue) valueWithVariable)
        .map(StringValue::getString);
  }
  ...
  public boolean classExists() {
    return Optional.ofNullable(directiveMap)
        ...
        .flatMap(
            directive ->
                Optional.ofNullable(directive.getArgument(DIRECTIVE_CLASS_ARGUMENT_EXISTS_NAME)))
  ...
  public Optional<String> getAnnotationName() {
    return Optional.ofNullable(directiveMap)
        ...
        .flatMap(
            directive ->
                Optional.ofNullable(directive.getArgument(DIRECTIVE_ANNOTATION_ARGUMENT_NAME_NAME)))
        .filter(ValueWithVariable::isString)
        .map(valueWithVariable -> (StringValue) valueWithVariable)
        .map(StringValue::getString);
  }
  ...
  public Optional<String> getGrpcName() {
    return Optional.ofNullable(directiveMap)
        ...
        .flatMap(
            directive ->
                Optional.ofNullable(directive.getArgument(DIRECTIVE_GRPC_ARGUMENT_NAME_NAME)))
        .filter(ValueWithVariable::isString)
        .map(valueWithVariable -> (StringValue) valueWithVariable)
        .map(StringValue::getString);
  }
  ...
  public String getAnnotationNameOrError() {
    return getAnnotationName()
        .orElseThrow(() -> new GraphQLErrors(ANNOTATION_NAME_ARGUMENT_NOT_EXIST));
  }
  ...
  public String getToolName() {
    return Optional.ofNullable(directiveMap)
        ...
        .flatMap(
            directive ->
                Optional.ofNullable(directive.getArgument(DIRECTIVE_TOOL_ARGUMENT_NAME_NAME)))
        .filter(ValueWithVariable::isString)
        .map(valueWithVariable -> (StringValue) valueWithVariable)
        .map(StringValue::getString)
        .orElseGet(() -> getName());
  ...
  public String getToolDescription() {
    return Optional.ofNullable(directiveMap)
        ...
        .flatMap(
            directive ->
                Optional.ofNullable(
                    directive.getArgument(DIRECTIVE_TOOL_ARGUMENT_DESCRIPTION_NAME)))
        .filter(ValueWithVariable::isString)
        .map(valueWithVariable -> (StringValue) valueWithVariable)
        .map(StringValue::getString)
