...
public class ObjectValueWithVariable extends AbstractMap<String, JsonValue>
    implements ValueWithVariable, JsonObject, Iterable<JsonValue> {

  private final STGroup stGroupFile =
      StringTemplateLoader.load("stg/common/ObjectValueWithVariable.stg");
  ...
  public ObjectValueWithVariable(Map<?, ?> objectValueWithVariable) {
    this.objectValueWithVariable =
        objectValueWithVariable.entrySet().stream()
            .collect(
                Collectors.toMap(
  ...
  public ObjectValueWithVariable(AnnotationMirror objectValueWithVariable) {
    this.objectValueWithVariable =
        objectValueWithVariable.getElementValues().entrySet().stream()
            .collect(
                Collectors.toMap(
        ...
        objectValueWithVariableContext.objectFieldWithVariable().stream()
            .collect(
                Collectors.toMap(
                    ...
                    objectFieldWithVariableContext ->
                        ValueWithVariable.of(objectFieldWithVariableContext.valueWithVariable()),
                    (x, y) -> y,
                    LinkedHashMap::new));
  }
  ...
  public ValueWithVariable getValueWithVariableOrError(String name) {
    return getValueWithVariableOrEmpty(name)
        .orElseThrow(() -> new GraphQLErrors(GraphQLErrorType.ARGUMENT_NOT_EXIST.bind(name)));
  }
  ...
  public static <K, V> ObjectValueWithVariable of(
