...
public class Operation extends AbstractDefinition implements Definition {
  ...
  public VariableDefinition getVariableDefinition(String name) {
    return Optional.ofNullable(variableDefinitionMap)
        .map(stringVariableDefinitionMap -> stringVariableDefinitionMap.get(name))
        .orElse(null);
  }
  ...
  public Operation addVariableDefinitions(Collection<VariableDefinition> variableDefinitions) {
    ...
    this.variableDefinitionMap.putAll(
        (Map<? extends String, ? extends VariableDefinition>)
            variableDefinitions.stream()
                .collect(
                    Collectors.toMap(
                        variableDefinition -> variableDefinition.getVariable().getName(),
  ...
  public Field getField(String name) {
    return getSelections().stream()
        ...
        .filter(
            field ->
                field.getAlias() != null && field.getAlias().equals(name)
                ...
                (pre, cur) ->
                    Stream.concat(
                            Stream.ofNullable(pre)
                                ...
                                .map(
                                    original -> {
                                      if (Optional.ofNullable(original.getAlias())
                                          .orElseGet(original::getName)
                                          .equals(
                                              Optional.ofNullable(cur.getAlias())
                                                  .orElseGet(cur::getName)))
                                ...
                                .filter(
                                    field ->
                                        Stream.ofNullable(pre)
                                            ...
                                            .noneMatch(
                                                original ->
                                                    Optional.ofNullable(original.getAlias())
                                                        .orElseGet(original::getName)
                                                        .equals(
                                                            Optional.ofNullable(field.getAlias())
  ...
  public Optional<Integer> getInvokeIndexName() {
    return Optional.ofNullable(getDirective(DIRECTIVE_INVOKE_NAME))
        .flatMap(
            directive -> directive.getArgumentOrEmpty(DIRECTIVE_INVOKE_ARGUMENT_METHOD_INDEX_NAME))
  ...
  public Stream<String> getInvokeThrownTypes() {
    return Stream.ofNullable(getDirective(DIRECTIVE_INVOKE_NAME))
        .flatMap(
            directive ->
                directive.getArgumentOrEmpty(DIRECTIVE_INVOKE_ARGUMENT_THROWN_TYPES_NAME).stream())
        .filter(ValueWithVariable::isArray)
