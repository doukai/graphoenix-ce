...
public class EnumType extends AbstractDefinition implements Definition {
  ...
  public EnumType(GraphqlParser.EnumTypeDefinitionContext enumTypeDefinitionContext) {
    super(
        enumTypeDefinitionContext.name(),
        enumTypeDefinitionContext.description(),
        enumTypeDefinitionContext.directives());
  ...
  public EnumType(
      GraphqlParser.EnumTypeExtensionDefinitionContext enumTypeExtensionDefinitionContext) {
    super(
        enumTypeExtensionDefinitionContext.name(),
        null,
        enumTypeExtensionDefinitionContext.directives());
    setExtension(true);
  ...
  public EnumType merge(
      GraphqlParser.EnumTypeExtensionDefinitionContext... enumTypeExtensionDefinitionContexts) {
    return merge(
        Stream.of(enumTypeExtensionDefinitionContexts).map(EnumType::new).toArray(EnumType[]::new));
  }

  public EnumType merge(EnumType... enumTypes) {
    super.merge(enumTypes);
    enumValueMap.putAll(
        (Map<? extends String, ? extends EnumValueDefinition>)
            Stream.concat(
