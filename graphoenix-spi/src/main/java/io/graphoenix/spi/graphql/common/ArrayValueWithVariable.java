...
public class ArrayValueWithVariable extends AbstractList<JsonValue>
  ...
  public ArrayValueWithVariable(Arrays valueWithVariables) {
    this.valueWithVariables =
        Stream.of(valueWithVariables).map(ValueWithVariable::of).collect(Collectors.toList());
  }
  ...
  public ArrayValueWithVariable(GraphqlParser.ArrayValueContext arrayValueContext) {
    this.valueWithVariables =
        arrayValueContext.value().stream().map(ValueWithVariable::of).collect(Collectors.toList());
  }

  public ArrayValueWithVariable(Collection<?> valueWithVariables) {
    this.valueWithVariables =
        valueWithVariables.stream().map(ValueWithVariable::of).collect(Collectors.toList());
  }
  ...
  public <T extends ValueWithVariable> ValueWithVariable merge(T arrayValueWithVariable) {
    ...
    this.valueWithVariables.addAll(
        ((ArrayValueWithVariable) arrayValueWithVariable).getValueWithVariables());
    return this;
  }
