...
public final class NameUtil {
    ...
    if (name.startsWith(PREFIX_INTROSPECTION)) {
      return PREFIX_INTROSPECTION
          + CaseFormat.UPPER_CAMEL.to(
              CaseFormat.LOWER_CAMEL, name.replace(PREFIX_INTROSPECTION, ""));
    }
  ...
  public static String getTypeRefFieldName(String typeName) {
    return CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_CAMEL, typeNameToFieldName(typeName))
        + SUFFIX_REF;
  }
    ...
    if (name.startsWith(PREFIX_INTROSPECTION)) {
      return "intro"
          + CaseFormat.LOWER_CAMEL.to(
              CaseFormat.UPPER_CAMEL, name.replace(PREFIX_INTROSPECTION, ""));
    }
    return name;
  }
