...
public class DownloadRequestHandler extends BaseHandler implements GetHandler {
    ...
    @Inject
    public DownloadRequestHandler(FileHandler fileHandler, RequestBeanScoped requestBeanScoped, HttpServerConfig httpServerConfig) {
    ...
    public Publisher<Void> handle(HttpServerRequest request, HttpServerResponse response) {
        ...
        return ScopeEventPublisher.initialized(Maps.newHashMap(Map.of(REQUEST, request, RESPONSE, response)), RequestScoped.class)
