  ...
  public Publisher<Void> handle(HttpServerRequest request, HttpServerResponse response) {
    ...
    String accept =
        decoder
            .parameters()
            .getOrDefault(
                ACCEPT.toString(), Collections.singletonList(request.requestHeaders().get(ACCEPT)))
            .get(0);
    GraphQLRequest graphQLRequest =
        new GraphQLRequest(
            decoder.parameters().get("query").get(0),
            decoder.parameters().containsKey("operationName")
                ? decoder.parameters().get("operationName").get(0)
                : null,
            decoder.parameters().containsKey("variables")
                ? jsonProvider
                    .createReader(
                        new StringReader(
                            Objects.requireNonNull(decoder.parameters().get("variables").get(0))))
                    .readObject()
                : null);
    ...
    if (accept.contains(MimeType.Text.EVENT_STREAM)) {
      String token =
          Optional.ofNullable(request.requestHeaders().get("X-GraphQL-Event-Stream-Token"))
              .orElseGet(
                  () ->
                      decoder.parameters().containsKey("token")
                          ? decoder.parameters().get("token").get(0)
                          : null);
      String operationId =
          decoder.parameters().containsKey("operationId")
              ? decoder.parameters().get("operationId").get(0)
              : null;
      return response
          .sse()
          ...
          .addHeader(
              HttpHeaderNames.CONTENT_TYPE, HttpHeaderValues.TEXT_EVENT_STREAM + "; charset=utf-8")
              ...
              ScopeEventPublisher.initialized(
                      Maps.newHashMap(
                          Map.of(
                              ...
                              response,
                              OPERATION,
                              document.getOperationOrError())),
                      RequestScoped.class)
                  .doOnSuccess(
                      v -> requestBeanScoped.put(requestId, HttpServerRequest.class, request))
                  .doOnSuccess(
                      v -> requestBeanScoped.put(requestId, HttpServerResponse.class, response))
                  .doOnSuccess(v -> requestBeanScoped.put(requestId, Document.class, document))
                  .doOnSuccess(v -> requestBeanScoped.put(requestId, Operation.class, operation))
                  .thenMany(
                      Flux.from(
                          operationHandler.handle(
                              operation, graphQLRequest.getVariables(), token, operationId)))
                  .map(jsonValue -> next(jsonValue, operationId))
                  .onErrorResume(
                      throwable -> this.errorSSEHandler(throwable, response, operationId))
                  .map(
                      eventString ->
                          ByteBufAllocator.DEFAULT
                              .buffer()
                              .writeBytes(eventString.getBytes(StandardCharsets.UTF_8)))
