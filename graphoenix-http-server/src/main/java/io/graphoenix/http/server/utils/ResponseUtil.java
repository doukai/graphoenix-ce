...
public final class ResponseUtil {
  ...
  public static String success(String jsonString, String id) {
    ...
    responseBuilder.add(
        "data", jsonProvider.createReader(new StringReader(jsonString)).readValue());
  ...
  public static String success(Object object, String id) {
    JsonObjectBuilder responseBuilder = jsonProvider.createObjectBuilder();
    responseBuilder.add(
        "data", jsonProvider.createReader(new StringReader(jsonb.toJson(object))).read());
    StringWriter stringWriter = new StringWriter();
    if (id != null) {
      jsonProvider
          .createWriter(stringWriter)
  ...
  public static String error(GraphQLErrors graphQLErrors, String id) {
    ...
    if (graphQLErrors.getData() != null) {
      responseBuilder.add(
          "data",
          jsonProvider
              .createReader(new StringReader(jsonb.toJson(graphQLErrors.getData())))
              .read());
    }
    responseBuilder.add(
        "errors",
        jsonProvider
            .createReader(new StringReader(jsonb.toJson(graphQLErrors.getErrors())))
            .read());
    StringWriter stringWriter = new StringWriter();
    if (id != null) {
      jsonProvider
