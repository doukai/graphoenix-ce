...
public class DefaultSubscriptionDataListener implements SubscriptionDataListener {
  ...
  @Inject
  public DefaultSubscriptionDataListener(
      DocumentManager documentManager, ArgumentsToFilter argumentsToFilter) {
            ...
            .flatMap(
                field -> {
                  ...
                  if (fieldTypeDefinition.isObject() && !fieldTypeDefinition.isContainer()) {
                    return argumentsToFilter
                        .argumentsToMultipleFilter(fieldDefinition, field)
                        .stream()
                        .map(
                            filter ->
                                new AbstractMap.SimpleEntry<>(
                                    fieldTypeDefinition.getName(), filter.toString()));
                  }
                  return Stream.empty();
                })
            .collect(
            ...
            .flatMap(
                field ->
                    indexID(
                        ...
                        jsonValue
                            .asJsonObject()
                            .get(Optional.ofNullable(field.getAlias()).orElseGet(field::getName))))
            .collect(
                Collectors.groupingBy(
                    Map.Entry::getKey,
                    Collectors.mapping(Map.Entry::getValue, Collectors.toSet()))));
  ...
  private Stream<Map.Entry<String, String>> indexID(
      FieldDefinition fieldDefinition, JsonValue jsonValue) {
    ...
    if (jsonValue.getValueType().equals(JsonValue.ValueType.NULL)) {
      ...
      return jsonValue.asJsonArray().stream().flatMap(item -> indexID(fieldDefinition, item));
    } else {
      return Stream.concat(
          fieldTypeDefinition
              ...
              .filter(
                  idFieldDefinition ->
                      jsonValue.asJsonObject().containsKey(idFieldDefinition.getName()))
              .filter(
                  idFieldDefinition ->
                      !jsonValue.asJsonObject().isNull(idFieldDefinition.getName()))
              .stream()
              .map(
                  idFieldDefinition ->
                      new AbstractMap.SimpleEntry<>(
                          fieldTypeDefinition.getName(),
                          jsonValue.asJsonObject().getString(idFieldDefinition.getName()))),
          jsonValue.asJsonObject().entrySet().stream()
              .flatMap(
                  entry -> {
                    FieldDefinition subFieldDefinition =
                        fieldTypeDefinition.asObject().getFieldOrError(entry.getKey());
                ...
                typeIDMap.containsKey(typeName)
                    && (mutation.asJsonObject().containsKey(idFieldDefinition.getName())
                            ...
                            && typeIDMap
                                .get(typeName)
                                .contains(
                                    mutation.asJsonObject().getString(idFieldDefinition.getName()))
                        || mutation.asJsonObject().containsKey(INPUT_VALUE_WHERE_NAME)
                            ...
                            && mutation
                                ...
                                .containsKey(idFieldDefinition.getName())
                            && !mutation
                                .asJsonObject()
                                .getJsonObject(INPUT_VALUE_WHERE_NAME)
                                .isNull(idFieldDefinition.getName())
                            && mutation
                                .asJsonObject()
                                .getJsonObject(INPUT_VALUE_WHERE_NAME)
                                .getJsonObject(idFieldDefinition.getName())
                                .containsKey(INPUT_OPERATOR_INPUT_VALUE_VAL_NAME)
                            && !mutation
                                .asJsonObject()
                                .getJsonObject(INPUT_VALUE_WHERE_NAME)
                                .getJsonObject(idFieldDefinition.getName())
                                .isNull(INPUT_OPERATOR_INPUT_VALUE_VAL_NAME)
                            && typeIDMap
                                .get(typeName)
                                .contains(
                                    mutation
                                        ...
                                        .getJsonObject(INPUT_VALUE_WHERE_NAME)
                                        .getJsonObject(idFieldDefinition.getName())
                                        .getString(INPUT_OPERATOR_INPUT_VALUE_VAL_NAME))))
        .orElseGet(
            () ->
                    ...
                    .anyMatch(
                        entry -> {
                          FieldDefinition fieldDefinition =
                              objectType.getFieldOrError(entry.getKey());
                          if (entry.getValue().getValueType().equals(JsonValue.ValueType.ARRAY)) {
                            return entry.getValue().asJsonArray().stream()
                                .filter(
                                    item -> item.getValueType().equals(JsonValue.ValueType.OBJECT))
                                .anyMatch(
                                    item ->
                                        idChanged(
                                            documentManager
