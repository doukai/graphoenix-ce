...
public class R2DBCTransactionInterceptor {
  ...
  public Object aroundInvoke(InvocationContext invocationContext) {
    ...
    try {
      if (invocationContext.getContextData().containsKey("value")) {
        ...
        txType =
            (Transactional.TxType) Transactional.class.getDeclaredMethod("value").getDefaultValue();
      }
      if (invocationContext.getContextData().containsKey("rollbackOn")) {
        rollbackOn =
            (Class<? extends Exception>[]) invocationContext.getContextData().get("rollbackOn");
      } else {
        rollbackOn =
            (Class<? extends Exception>[])
                Transactional.class.getDeclaredMethod("rollbackOn").getDefaultValue();
      }
      if (invocationContext.getContextData().containsKey("dontRollbackOn")) {
        dontRollbackOn =
            (Class<? extends Exception>[]) invocationContext.getContextData().get("dontRollbackOn");
      } else {
        dontRollbackOn =
            (Class<? extends Exception>[])
                Transactional.class.getDeclaredMethod("dontRollbackOn").getDefaultValue();
      }
        ...
        switch (txType) {
          ...
          case REQUIRES_NEW:
            return Mono.usingWhen(
                    connectionProvider.get().contextWrite(Context.of(IN_TRANSACTION, false)),
                    connection ->
                        Mono.from(connection.setAutoCommit(false))
                            .then(Mono.from(connection.beginTransaction()))
                            .then(Mono.from(proceed(invocationContext))),
                    connection ->
                        Mono.from(connection.commitTransaction()).thenEmpty(connection.close()),
                    (connection, throwable) ->
                        Mono.from(errorProcess(connection, throwable, rollbackOn, dontRollbackOn))
                            .thenEmpty(connection.close())
                            .thenEmpty(Mono.error(throwable)),
                    connection ->
                        Mono.from(connection.rollbackTransaction()).thenEmpty(connection.close()))
                .contextWrite(
                    Context.of(
                        TRANSACTION_ID,
                        transactionId,
                    ...
                    inTransaction ->
                        inTransaction
                            ...
                            : Mono.usingWhen(
                                ...
                                .contextWrite(
                                    Context.of(
                                        TRANSACTION_ID, transactionId, IN_TRANSACTION, false)));
          case NOT_SUPPORTED:
            return connectionProvider
                .inTransaction()
                .flatMap(
                    inTransaction ->
                        Mono.usingWhen(
                                ...
                                Connection::close)
                            .contextWrite(
                                Context.of(TRANSACTION_ID, transactionId, IN_TRANSACTION, false)));
          case NEVER:
            return connectionProvider
                .inTransaction()
                .flatMap(
                    ...
                    inTransaction ->
                        inTransaction
                            ...
                            : Flux.usingWhen(
                                    ...
                                    connection ->
                                        Flux.from(connection.commitTransaction())
                                            .thenEmpty(connection.close()),
                                    (connection, throwable) ->
                                        Flux.from(
                                                errorProcess(
                                    ...
                                    connection ->
                                        Flux.from(connection.rollbackTransaction())
                                            .thenEmpty(connection.close()))
                                .contextWrite(
                                    Context.of(
                                        TRANSACTION_ID,
            ...
            return Flux.usingWhen(
                    ...
                    connection ->
                        Flux.from(connection.setAutoCommit(false))
                            ...
                            .thenMany(Flux.from(proceed(invocationContext))),
                    connection ->
                        Flux.from(connection.commitTransaction()).thenEmpty(connection.close()),
                    (connection, throwable) ->
                        Flux.from(errorProcess(connection, throwable, rollbackOn, dontRollbackOn))
                            .thenEmpty(connection.close())
                            .thenEmpty(Flux.error(throwable)),
                    connection ->
                        Flux.from(connection.rollbackTransaction()).thenEmpty(connection.close()))
                .contextWrite(
                    Context.of(
                        TRANSACTION_ID,
                        transactionId,
  ...
  private Publisher<Void> errorProcess(
