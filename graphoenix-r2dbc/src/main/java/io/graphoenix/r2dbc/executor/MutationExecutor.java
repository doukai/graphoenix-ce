...
public class MutationExecutor {
  ...
  public Flux<Long> executeMutationsInBatchByGroup(
      Stream<String> mutationSqlStream, int groupSize) {
  ...
  public Mono<Long> executeMutations(
      Stream<String> mutationSqlStream, Map<String, Object> parameters) {
    return Flux.fromStream(mutationSqlStream)
        .collectList()
        .filter(sqlList -> !sqlList.isEmpty())
        .flatMap(
  ...
  public Flux<Long> executeMutationsFlux(
      Stream<String> mutationSqlStream, Map<String, Object> parameters) {
    return Flux.usingWhen(
        connectionProvider.get(),
        connection ->
