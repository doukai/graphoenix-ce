...
public class R2DBCParameterHandler {
  ...
  public Map<String, Object> process(Map<String, Object> parameters) {
    return parameters.entrySet().stream()
        .collect(
            HashMap::new, (m, v) -> m.put(v.getKey(), processValue(v.getValue())), HashMap::putAll);
  }
    ...
    if (value == null) {
      ...
      return ((Map<String, ?>) value)
          .entrySet().stream()
              .map(
                  entry ->
                      new AbstractMap.SimpleEntry<>(entry.getKey(), toJsonValue(entry.getValue())))
              .collect(JsonCollectors.toJsonObject());
    } else if (value instanceof Collection) {
      return ((Collection<?>) value)
          .stream().map(this::toJsonValue).collect(JsonCollectors.toJsonArray());
    } else {
