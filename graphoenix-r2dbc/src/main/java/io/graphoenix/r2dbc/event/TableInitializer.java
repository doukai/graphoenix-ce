...
public class TableInitializer {
  ...
  public static final int TABLE_INITIALIZED_SCOPE_EVENT_PRIORITY =
      DOCUMENT_INITIALIZED_SCOPE_EVENT_PRIORITY + 100;
  ...
  public TableInitializer(
      R2DBCConfig r2DBCConfig, TypeTranslator typeTranslator, TableManager tableManager) {
    this.r2DBCConfig = r2DBCConfig;
    this.typeTranslator = typeTranslator;
    this.tableManager = tableManager;
  }

  public Mono<Void> initializeTable(
      @Observes
          @Initialized(ApplicationScoped.class)
          @Priority(TableInitializer.TABLE_INITIALIZED_SCOPE_EVENT_PRIORITY)
          Object event) {
    if (r2DBCConfig.getCreateTables()) {
      logger.info("table initialized starting");
      return tableManager
          .selectColumns(typeTranslator.selectColumnsSQL())
          .flatMap(
              existsColumnNameList ->
                  tableManager.mergeTable(
                      typeTranslator
                          .mergeTablesSQL(existsColumnNameList)
                          .collect(Collectors.joining(";"))))
          .doOnSuccess((v) -> logger.info("table initialized success"))
          .then(
              tableManager
                  .selectIndexes(typeTranslator.selectIndexesSQL())
                  .flatMap(
                      existsIndexNameList ->
                          tableManager.mergeTable(
                              typeTranslator
                                  .createIndexesSQL(existsIndexNameList)
                                  .collect(Collectors.joining(";"))))
