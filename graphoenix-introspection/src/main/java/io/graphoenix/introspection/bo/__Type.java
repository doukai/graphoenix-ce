...
public class __Type {
  ...
  public ObjectValueWithVariable toObjectValue() {
    ...
    if (this.getFields() != null) {
      objectValueWithVariable.put(
          "fields",
          this.getFields().stream().map(__Field::toObjectValue).collect(Collectors.toList()));
    }
    if (this.getEnumValues() != null) {
      objectValueWithVariable.put(
          "enumValues",
          this.getEnumValues().stream()
              .map(__EnumValue::toObjectValue)
              .collect(Collectors.toList()));
    }
    if (this.getInputFields() != null) {
      objectValueWithVariable.put(
          "inputFields",
          this.getInputFields().stream()
              .map(__InputValue::toObjectValue)
              .collect(Collectors.toList()));
    }
