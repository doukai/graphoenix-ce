...
public class IntrospectionCleaner {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(IntrospectionCleaner.class.getName());

  public static final int INTROSPECTION_CLEAN_SCOPE_EVENT_PRIORITY =
      INTROSPECTION_BUILD_SCOPE_EVENT_PRIORITY - 1;
  ...
  @Inject
  public IntrospectionCleaner(
      GraphQLConfig graphQLConfig,
      DocumentManager documentManager,
      TypeEmptyHandler typeEmptyHandler) {
    this.graphQLConfig = graphQLConfig;
    this.documentManager = documentManager;
    this.typeEmptyHandler = typeEmptyHandler;
  }

  public Mono<Void> cleanIntrospection(
      @Observes
          @Initialized(ApplicationScoped.class)
          @Priority(IntrospectionCleaner.INTROSPECTION_CLEAN_SCOPE_EVENT_PRIORITY)
          Object event) {
