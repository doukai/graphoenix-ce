...
public class IntrospectionMutationBuilder {
  ...
  public Operation buildIntrospectionSchemaMutation() {
    ...
    List<ObjectValueWithVariable> interfacesObjectValues =
        documentManager
            .getDocument()
            .getObjectTypes()
            .map(this::buildType)
    ...
    List<ObjectValueWithVariable> possibleTypesObjectValues =
        documentManager
            .getDocument()
            .getInterfaceTypes()
            .map(this::buildType)
    ...
    schema.setTypes(
        Streams.concat(
                ...
                documentManager
                    ...
                    .getInterfaceTypes()
                    .filter(
                        interfaceType -> !interfaceType.getName().startsWith(PREFIX_INTROSPECTION))
                    .map(this::buildType),
                documentManager
                    .getDocument()
                    .getInputObjectTypes()
                    .filter(
                        inputObjectType ->
                            !inputObjectType.getName().startsWith(PREFIX_INTROSPECTION))
                    .map(this::buildType),
                documentManager
                    .getDocument()
                    .getEnums()
                    ...
                    .map(this::buildType),
                documentManager.getDocument().getScalarTypes().map(this::buildType))
    ...
    if (level == 0) {
      ...
      __type.setFields(
          objectType.getFields().stream()
              .filter(
                  fieldDefinition -> !fieldDefinition.getName().startsWith(PREFIX_INTROSPECTION))
  ...
  private __Type buildType(InterfaceType interfaceType, int level) {
    ...
    if (level == 0) {
      if (interfaceType.getInterfaces() != null) {
        __type.setInterfaces(
            getInterfaces(interfaceType.getInterfaces(), level + 1)
                .collect(Collectors.toCollection(LinkedHashSet::new)));
      } else {
        __type.setInterfaces(new LinkedHashSet<>());
      }
    ...
    return implementsInterfaceNames.stream()
        .flatMap(
            implementsInterfaceName ->
                documentManager.getDocument().getInterfaceType(implementsInterfaceName).stream())
              ...
              .filter(
                  inputValue ->
                      !documentManager
                          ...
                          .getName()
                          .startsWith(PREFIX_INTROSPECTION))
  ...
  private __Type buildType(Type type, int level) {
    ...
    if (type.isList()) {
      ...
      Definition definition =
          documentManager.getDocument().getDefinition(type.asTypeName().getName());
  ...
  private __Directive buildDirective(DirectiveDefinition directiveDefinition) {
    ...
    __directive.setLocations(
        buildDirectiveLocationList(directiveDefinition.getDirectiveLocations()));
    return __directive;
  }

  public Set<__DirectiveLocation> buildDirectiveLocationList(
      Collection<String> directiveLocationNames) {
