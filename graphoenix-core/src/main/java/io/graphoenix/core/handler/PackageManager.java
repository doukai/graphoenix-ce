  ...
  public PackageManager(PackageConfig packageConfig) {
    ...
    if (!getSeedMembers().isEmpty()) {
      this.packageProvider =
          CDI.current()
              .select(PackageProvider.class, NamedLiteral.of(PACKAGE_PROVIDER_GOSSIP_NAME))
              .get();
    } else {
      this.packageProvider =
          CDI.current().select(PackageProvider.class, Default.Literal.INSTANCE).get();
    }
  }
          ...
          .or(
              () ->
                  classPath.getTopLevelClasses().stream()
                      .filter(
                          classInfo ->
                              classInfo.load().getPackage().isAnnotationPresent(Application.class))
                      .findFirst())
