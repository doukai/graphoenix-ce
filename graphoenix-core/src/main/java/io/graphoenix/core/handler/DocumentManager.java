...
public class DocumentManager {
  ...
  public Document getPackageDocument() {
    return document.setDefinitions(
  ...
  public boolean isQueryOperationType(Definition definition) {
    return definition.isObject()
        && definition
            .getName()
            .equals(document.getSchema().map(Schema::getQuery).orElse(TYPE_QUERY_NAME));
  }
  ...
  public boolean isMutationOperationType(Definition definition) {
    return definition.isObject()
        && definition
            .getName()
            .equals(document.getSchema().map(Schema::getMutation).orElse(TYPE_MUTATION_NAME));
  }
  ...
  public boolean isSubscriptionOperationType(Definition definition) {
    return definition.isObject()
        && definition
            .getName()
            .equals(
                document.getSchema().map(Schema::getSubscription).orElse(TYPE_SUBSCRIPTION_NAME));
  }

  public boolean isOperationType(Definition definition) {
    return isQueryOperationType(definition)
        || isMutationOperationType(definition)
        || isSubscriptionOperationType(definition);
  }
    ...
    return Optional.ofNullable(document.getDefinition(INTERFACE_META_NAME))
        ...
        .anyMatch(
            interfaceFieldDefinition ->
                interfaceFieldDefinition.getName().equals(fieldDefinition.getName()));
  }
  ...
  public boolean isMetaExpressInputValue(InputValue inputValue) {
    return Optional.ofNullable(document.getDefinition(INTERFACE_META_NAME + SUFFIX_EXPRESSION))
        .map(definition -> (InputObjectType) definition)
        .stream()
  ...
  public FieldDefinition getFieldMapFromFieldDefinition(
  ...
  public Optional<FieldDefinition> getFieldMapToFieldDefinition(FieldDefinition fieldDefinition) {
    return fieldDefinition
        .getMapTo()
        .map(mapTo -> getFieldTypeDefinition(fieldDefinition).asObject().getField(mapTo));
  }
  ...
  public FieldDefinition getFieldFetchFromFieldDefinition(
      ObjectType objectType, FieldDefinition fieldDefinition) {
  ...
  public Definition getInputBelong(Definition definition) {
    ...
    if (definition.getName().endsWith(SUFFIX_INPUT)) {
      ...
      typeName =
          definition.getName().substring(0, definition.getName().lastIndexOf(SUFFIX_EXPRESSION));
    } else {
        ...
        typeName =
            definition
                ...
                .substring(
                    ...
                    definition
                        .getName()
                        .lastIndexOf(SUFFIX_LIST + queryTypeName + SUFFIX_ARGUMENTS));
      } else if (definition.getName().endsWith(queryTypeName + SUFFIX_ARGUMENTS)) {
        typeName =
            definition
                .getName()
                .substring(0, definition.getName().lastIndexOf(queryTypeName + SUFFIX_ARGUMENTS));
      }
        ...
        typeName =
            definition
                ...
                .substring(
                    ...
                    definition
                        .getName()
                        .lastIndexOf(SUFFIX_LIST + mutationTypeName + SUFFIX_ARGUMENTS));
      } else if (definition.getName().endsWith(mutationTypeName + SUFFIX_ARGUMENTS)) {
        typeName =
            definition
                .getName()
                .substring(
                    0, definition.getName().lastIndexOf(mutationTypeName + SUFFIX_ARGUMENTS));
      }
        ...
        typeName =
            definition
                ...
                .substring(
                    ...
                    definition
                        .getName()
                        .lastIndexOf(SUFFIX_LIST + subscriptionTypeName + SUFFIX_ARGUMENTS));
      } else if (definition.getName().endsWith(subscriptionTypeName + SUFFIX_ARGUMENTS)) {
        typeName =
            definition
                .getName()
                .substring(
                    0, definition.getName().lastIndexOf(subscriptionTypeName + SUFFIX_ARGUMENTS));
      }
    }
    if (typeName != null) {
                    ...
                    .flatMap(
                        inputValue ->
                            inputValue
                                .getDefaultValue()
                                .asObject()
                                .getValueWithVariableOrEmpty(DIRECTIVE_INVOKES_METHODS_NAME)
                                .stream()
