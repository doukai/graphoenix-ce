...
public class TransactionCompensatorBackupHandler implements OperationBeforeHandler {
  ...
  public static final int TRANSACTION_COMPENSATOR_BACKUP_HANDLER_PRIORITY =
      MUTATION_BEFORE_FETCH_HANDLER_PRIORITY - 1;
  ...
  @Inject
  public TransactionCompensatorBackupHandler(
  ...
  public Mono<Operation> mutation(Operation operation, Map<String, JsonValue> variables) {
    ...
    return transactionCompensatorProvider
        .get()
        .flatMap(
              ...
              List<FetchItem> fetchItemList =
                  operation.getFields().stream()
                      .flatMap(
                          field ->
                              buildFetchItems(
                                  operationType.getFieldOrError(field.getName()), field))
                          ...
                          .collect(
                              Collectors.groupingBy(
                                  ...
                                  Collectors.mapping(
                                      fetchItem -> fetchItem,
                                      Collectors.groupingBy(
                                          FetchItem::getProtocol, Collectors.toList()))))
                          .entrySet())
                  .flatMap(
                      packageEntries ->
                                      ...
                                      Mono.just(
                                          ...
                                          .flatMap(
                                              fieldList ->
                                                  CDI.current()
                                                      .select(
                                                          PackageFetchHandler.class,
                                                          NamedLiteral.of(protocolEntries.getKey()))
                                                      .get()
                                                      .request(
                                                          packageEntries.getKey(),
                                                          new Operation()
                                                              .setOperationType(
                                                                  OPERATION_QUERY_NAME)
                                                              .setSelections(fieldList)))
                                          .flatMapMany(
                                              fetchJsonValue ->
                                                  Flux.fromIterable(protocolEntries.getValue())
                                                      .filter(
                                                          fetchItem ->
                                                              fetchItem.getFetchField() != null)
                                                      .map(
                                                          fetchItem ->
                  ...
                  .map(
                      entryList ->
                          transactionCompensator
                              ...
                              .addTypeFetchItemListMap(
                                  fetchItemList.stream()
                                      .filter(fetchItem -> fetchItem.getFetchField() == null)
                                      .filter(fetchItem -> fetchItem.getTarget() == null)
                                      .collect(
                                          Collectors.groupingBy(
                                              FetchItem::getTypeName, Collectors.toList())))
                              .addRelationTypeValueListMap(
                                  fetchItemList.stream()
                                      .filter(fetchItem -> fetchItem.getFetchField() == null)
                                      .filter(fetchItem -> fetchItem.getTarget() != null)
                                      .collect(
    ...
    if (fieldTypeDefinition.isObject() && !fieldTypeDefinition.isContainer()) {
      ...
      if (!packageManager.isLocalPackage(packageName)) {
        if (field.getArguments().hasArgument(idField.getName())
            || field.getArguments().hasArgument(INPUT_VALUE_WHERE_NAME)) {
          Field fetchField =
              new Field(field.getName())
                  ...
                  .setSelections(
                      field.getArguments().getArguments().keySet().stream()
                          .flatMap(
                              key ->
                                  Stream.ofNullable(fieldTypeDefinition.asObject().getField(key)))
                          .filter(
                              subFieldDefinition ->
                                  documentManager
          ...
          if (input.containsKey(idField.getName()) || input.containsKey(INPUT_VALUE_WHERE_NAME)) {
            ...
            fetchItemStream =
                Stream.of(
                    new FetchItem(
                        packageName, protocol, fieldTypeDefinition.getName(), fetchField));
          } else {
            fetchItemStream =
                Stream.of(
                    new FetchItem(
                  ...
                  .mapToObj(
                      index -> {
                        ...
                        if (item.containsKey(idField.getName())
                            || item.containsKey(INPUT_VALUE_WHERE_NAME)) {
                          Field fetchField =
                              new Field(field.getName())
                                  ...
                                  .setSelections(
                                      item.getObjectValueWithVariable().keySet().stream()
                                          .flatMap(
                                              key ->
                                                  Stream.ofNullable(
                                                      fieldTypeDefinition.asObject().getField(key)))
                                          .filter(
                                              subFieldDefinition ->
                                                  documentManager
                                                      .getFieldTypeDefinition(subFieldDefinition)
                                          ...
                                          .map(
                                              subFieldDefinition ->
                                                  new Field(subFieldDefinition.getName()))
                                          .collect(Collectors.toList()))
                                  .mergeSelection(new Field(idField.getName()))
                                  .mergeSelection(new Field(FIELD_DEPRECATED_NAME));
                          if (item.containsKey(idField.getName())) {
      ...
      Stream<FetchItem> subFetchItemStream =
          Streams.concat(
                                          ...
                                          .flatMap(
                                              subInputValue ->
                                                  Stream.ofNullable(
                                                          fieldTypeDefinition
                                                              .asObject()
                                                              .getField(subInputValue.getName()))
                                                      .flatMap(
                                                          subFieldDefinition ->
                                                              objectValueWithVariable
                                  ...
                                  arguments ->
                                      arguments
                                          ...
                                          .or(
                                              () ->
                                                  Optional.ofNullable(
                                                      listInputValue.getDefaultValue()))
                                          .stream())
  ...
  public Stream<FetchItem> buildFetchItems(
      ObjectType objectType,
      ValueWithVariable parentIDValueWithVariable,
      Field field,
      String path,
    ...
    if (fieldTypeDefinition.isObject() && !fieldTypeDefinition.isContainer()) {
      ...
      String protocol =
          packageManager.isLocalPackage(packageName)
              ? ENUM_PROTOCOL_ENUM_VALUE_LOCAL
              : fieldDefinition
                  .getFetchProtocol()
                  .map(EnumValue::getValue)
        ...
        if (fieldDefinition.getType().hasList()) {
          ...
          if (valueWithVariable.asObject().containsKey(idField.getName())
            ...
            Field fetchField =
                new Field()
                    .setAlias(
                        alias + "__" + getAliasFromPath(path) + "_" + fieldDefinition.getName())
                    .setSelections(
      ...
      if (fieldDefinition.hasFetchWith() || fieldDefinition.hasMapWith()) {
        ...
        if (!packageManager.isLocalPackage(withTypePackageName)) {
          ...
          String fetchWithFrom =
              fieldDefinition.getFetchWithFrom().orElseGet(fieldDefinition::getMapWithFromOrError);

          FieldDefinition refFieldDefinition =
              objectType.getFieldOrError(typeNameToFieldName(fetchWithType.getName()));
          String withTypeProtocol =
              packageManager.isLocalPackage(withTypePackageName)
                  ? ENUM_PROTOCOL_ENUM_VALUE_LOCAL
                  : packageConfig.getDefaultFetchProtocol();
          String target =
              fetchWithType.getFields().stream()
                  .filter(
                      withTypeFieldDefinition ->
                          Stream.concat(
                  ...
                  .orElseThrow(
                      () ->
                          new GraphQLErrors(
                              FETCH_WITH_TO_OBJECT_FIELD_NOT_EXIST.bind(fetchWithFrom)));

          if (parentIDValueWithVariable != null) {
            Field fetchField =
                new Field(typeNameToFieldName(fetchWithType.getName()) + SUFFIX_LIST)
                    .setAlias(
                        alias + "__" + getAliasFromPath(path) + "_" + refFieldDefinition.getName())
                                ...
                                if (fieldDefinition.getType().hasList()) {
                                  ...
                                  return Stream.ofNullable(
                                      ...
                                      .flatMap(
                                          objectValue ->
                                              Optional.ofNullable(
                                                  objectValue.get(subInputValue.getName()))
                                                  .or(
                                                      () ->
                                                          Optional.ofNullable(
