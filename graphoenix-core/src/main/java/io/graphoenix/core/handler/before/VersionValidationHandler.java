...
public class VersionValidationHandler implements OperationBeforeHandler {
  ...
  public static final int VERSION_VALIDATION_HANDLER_PRIORITY =
      UNIQUE_VALIDATION_HANDLER_PRIORITY + 1;
  ...
  @Inject
  public VersionValidationHandler(
  ...
  public Mono<Operation> mutation(Operation operation, Map<String, JsonValue> variables) {
    ...
    return Flux.fromStream(
            operation.getFields().stream()
                .flatMap(
                    field ->
                        buildVersionItems(operationType.getFieldOrError(field.getName()), field))
                .collect(Collectors.groupingBy(Tuple5::getT1, Collectors.toList()))
                .entrySet()
                .stream()
                .map(
                    typeEntry ->
                        new AbstractMap.SimpleEntry<>(
                            new Field(
                                    typeNameToFieldName(typeEntry.getKey().getName()) + SUFFIX_LIST)
                                .setAlias(
                                    typeNameToFieldName(typeEntry.getKey().getName()) + SUFFIX_LIST)
                                .setArguments(
                                    jsonProvider
                                        .createObjectBuilder()
                                        .add(
                                            typeEntry.getKey().getIDFieldOrError().getName(),
                                            jsonProvider
                                                .createObjectBuilder()
                                                .add(
                                        ...
                                        .build())
                                .addSelection(
                                    new Field(typeEntry.getKey().getIDFieldOrError().getName()))
                                ...
                                typeEntryList.stream()
                                    .filter(
                                        typeEntry ->
                                            !jsonValue
                                                .asJsonObject()
                                                .isNull(typeEntry.getKey().getAlias()))
                                    .flatMap(
                                        typeEntry ->
                                            typeEntry.getValue().stream()
                                                .filter(
                                                ...
                                                .map(
                                                    tuple5 ->
                                                        new GraphQLError(RESOURCE_HAS_BEEN_UPDATED)
                                                            .setPath(tuple5.getT4()))))))
        .collectList()
        .flatMap(
            graphQLErrors -> {
              if (!graphQLErrors.isEmpty()) {
                //                                return Mono.error(new
                // GraphQLErrors().addAll(graphQLErrors));
  ...
  public Stream<Tuple5<ObjectType, String, ValueWithVariable, String, ValueWithVariable>>
      buildVersionItems(FieldDefinition fieldDefinition, Field field) {
    ...
    if (fieldTypeDefinition.isObject() && !fieldTypeDefinition.isContainer()) {
      ...
      ValueWithVariable idValueWithVariable =
          getIDValueWithVariable(idField.getName(), field.getArguments()).orElse(new NullValue());
                              ...
                              objectValueWithVariable -> {
                                ValueWithVariable itemIdValueWithVariable =
                                    getIDValueWithVariable(
                                            idField.getName(), objectValueWithVariable)
                          ...
                          .flatMap(
                              arguments ->
                                  arguments
                                      ...
                                      .or(
                                          () ->
                                              Optional.ofNullable(listInputValue.getDefaultValue()))
                                      .stream())
