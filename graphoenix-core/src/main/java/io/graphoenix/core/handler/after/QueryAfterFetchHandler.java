    ...
    if (documentManager.isQueryOperationType(objectType)
      ...
      if (fieldDefinition.hasFetchWith()) {
        ...
        fetchField
                ...
                Streams.concat(
                        ...
                        fieldDefinition.getArguments().stream()
                            .filter(
                                inputValue ->
                                    Stream.of(
                                            INPUT_VALUE_FIRST_NAME,
                                            INPUT_VALUE_LAST_NAME,
                                            INPUT_VALUE_OFFSET_NAME,
                                            INPUT_VALUE_BEFORE_NAME,
                                            INPUT_VALUE_AFTER_NAME)
                        ...
                        Optional.of(
                            fieldDefinition.getArguments().stream()
                                ...
                                .collect(
                                    Collectors.toMap(
                                        Map.Entry::getKey, Map.Entry::getValue, (x, y) -> y)))
            ...
            .setName(
                typeNameToFieldName(fetchWithType.getName())
                    + (fieldDefinition.getType().hasList() ? SUFFIX_LIST : ""));
      ...
      if (!jsonValue.asJsonObject().containsKey(selectionName)
          || jsonValue.asJsonObject().isNull(selectionName)) {
        return Stream.empty();
      }
