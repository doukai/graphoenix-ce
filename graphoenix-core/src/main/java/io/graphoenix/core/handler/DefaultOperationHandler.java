...
public class DefaultOperationHandler implements OperationHandler {
  ...
  public DefaultOperationHandler(
    ...
    this.operationBeforeHandlerList =
        operationBeforeHandlerInstance.stream().collect(Collectors.toList());
    this.operationAfterHandlerList =
        operationAfterHandlerInstance.stream().collect(Collectors.toList());
    this.queryHandler =
        Optional.ofNullable(graphQLConfig.getDefaultOperationHandlerName())
    ...
    this.subscriptionHandler =
        Optional.ofNullable(graphQLConfig.getDefaultOperationHandlerName())
            .map(
                name ->
                    CDI.current().select(SubscriptionHandler.class, NamedLiteral.of(name)).get())
  ...
  @Override
  public Publisher<JsonValue> handle(
  ...
  public Mono<JsonValue> query(Operation operation, Map<String, JsonValue> variables) {
    return Flux.fromIterable(operationBeforeHandlerList)
        .reduce(
            Mono.just(operation), (pre, cur) -> pre.flatMap(result -> cur.query(result, variables)))
        .flatMap(operationMono -> operationMono)
        .flatMap(
            operationAfterHandler ->
                queryHandler
                    .query(operationAfterHandler)
                    .flatMap(
                        jsonValue ->
                            Flux.fromIterable(operationAfterHandlerList)
                                .reduce(
                                    ...
                                    (pre, cur) ->
                                        pre.flatMap(
                                            result -> cur.query(operationAfterHandler, result)))
                                .flatMap(operationMono -> operationMono)))
  ...
  public Mono<JsonValue> mutation(Operation operation, Map<String, JsonValue> variables) {
    return Flux.fromIterable(operationBeforeHandlerList)
        ...
        .flatMap(
            operationAfterHandler ->
                mutationHandler
                    .mutation(operationAfterHandler)
                    .flatMap(
                        jsonValue ->
                            Flux.fromIterable(operationAfterHandlerList)
                                .reduce(
                                        ...
                                        pre.flatMap(
                                            result ->
                                                cur.mutation(operationAfterHandler, result)
                                                    .onErrorResume(
                                                        throwable ->
                                                            transactionCompensatorProvider
                                                                .get()
                                                                .flatMap(
  ...
  public Flux<JsonValue> subscription(
      Operation operation, Map<String, JsonValue> variables, String token, String operationId) {
    return Flux.fromIterable(operationBeforeHandlerList)
        .reduce(
            Mono.just(operation),
        ...
        .flatMapMany(
            operationAfterHandler ->
                subscriptionHandler
                    .subscription(operationAfterHandler, token, operationId)
                    .flatMap(
                        jsonValue ->
                            Flux.fromIterable(operationAfterHandlerList)
                                .reduce(
                                    ...
                                    (pre, cur) ->
                                        pre.flatMap(
                                            result ->
                                                cur.subscription(operationAfterHandler, result)))
                                .flatMap(operationMono -> operationMono)))
        .defaultIfEmpty(JsonValue.EMPTY_JSON_OBJECT);
  }
}
