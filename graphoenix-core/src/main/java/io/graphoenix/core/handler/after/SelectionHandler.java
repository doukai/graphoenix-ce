...
public class SelectionHandler implements OperationAfterHandler {
                ...
                operation.getFields().stream()
                    ...
                    .filter(
                        StreamUtil.distinctByKey(
                            jsonObject ->
                                jsonObject.getString("op") + jsonObject.getString("path")))
                    .collect(JsonCollectors.toJsonArray()))
            .build()
            .apply(jsonValue.asJsonObject()));
  }

  public Stream<JsonObject> buildFormat(
            ...
            .flatMap(
                subField -> {
                  ...
                  return buildFormat(
                      ...
                      subField,
                      jsonValue.asJsonObject().get(subSelectionName));
                });
      }
    } else {
      if (field.hasFormat()) {
                  ...
                  index ->
                      jsonProvider
                          ...
                          .add(
                              ...
                              formatField(
                                  fieldTypeDefinition,
                                  field.getFormatValueOrNull(),
                                  field.getFormatLocaleOrNull(),
                  ...
                  index ->
                      jsonProvider
                          ...
                          .add(
                              ...
                              formatField(
                                  fieldTypeDefinition,
                                  fieldDefinition.getFormatValueOrNull(),
                                  fieldDefinition.getFormatLocaleOrNull(),
                                  jsonValue.asJsonArray().get(index)))
                          .build());
        } else {
          return Stream.of(
  ...
  public JsonValue formatField(
      Definition fieldTypeDefinition, String value, String locale, JsonValue jsonValue) {
    switch (fieldTypeDefinition.getName()) {
      case SCALA_BOOLEAN_NAME:
        return scalarFormatter.format(
            value, locale, jsonb.fromJson(jsonValue.toString(), Boolean.class));
      case SCALA_ID_NAME:
      case SCALA_STRING_NAME:
      case SCALA_UPLOAD_NAME:
        return scalarFormatter.format(
            value, locale, jsonb.fromJson(jsonValue.toString(), String.class));
      case SCALA_DATE_NAME:
        return scalarFormatter.format(
            value, locale, jsonb.fromJson(jsonValue.toString(), LocalDate.class));
      case SCALA_TIME_NAME:
        return scalarFormatter.format(
            value, locale, jsonb.fromJson(jsonValue.toString(), LocalTime.class));
      case SCALA_DATE_TIME_NAME:
      case SCALA_TIMESTAMP_NAME:
        return scalarFormatter.format(
            value, locale, jsonb.fromJson(jsonValue.toString(), LocalDateTime.class));
      case SCALA_INT_NAME:
        return scalarFormatter.format(
            value, locale, jsonb.fromJson(jsonValue.toString(), Integer.class));
      case SCALA_BIG_INTEGER_NAME:
        return scalarFormatter.format(
            value, locale, jsonb.fromJson(jsonValue.toString(), BigInteger.class));
      case SCALA_FLOAT_NAME:
        return scalarFormatter.format(
            value, locale, jsonb.fromJson(jsonValue.toString(), Float.class));
      case SCALA_BIG_DECIMAL_NAME:
        return scalarFormatter.format(
            value, locale, jsonb.fromJson(jsonValue.toString(), BigDecimal.class));
      default:
        throw new GraphQLErrors(
            GraphQLErrorType.UNSUPPORTED_FIELD_TYPE.bind(fieldTypeDefinition.toString()));
    }
  }

  public Stream<JsonObject> hideField(
      String path, FieldDefinition fieldDefinition, Field field, JsonValue jsonValue) {
    if (jsonValue.getValueType().equals(JsonValue.ValueType.NULL)) {
      return Stream.empty();
    }
    if (field.isHide()) {
      return Stream.of(
          jsonProvider.createObjectBuilder().add("op", "remove").add("path", path).build());
    } else {
      Definition fieldTypeDefinition = documentManager.getFieldTypeDefinition(fieldDefinition);
      if (fieldTypeDefinition.isObject()) {
        if (fieldDefinition.getType().hasList()) {
                              ...
                              .filter(
                                  key ->
                                      field.getFields().stream()
                                          .map(
                                              item ->
                                                  Optional.ofNullable(item.getAlias())
                  ...
                  .filter(
                      key ->
                          field.getFields().stream()
                              .map(
                                  item ->
                                      Optional.ofNullable(item.getAlias()).orElseGet(item::getName))
                              .noneMatch(alias -> alias.equals(key)))
                  .map(
                      key ->
                          jsonProvider
