...
public class MutationAfterFetchHandler implements OperationAfterHandler, FetchAfterHandler {
  ...
  @Inject
  public MutationAfterFetchHandler(
                    ...
                    Collectors.groupingBy(
                        ...
                        Collectors.mapping(
                            ...
                            Collectors.groupingBy(
                                ...
                                Collectors.mapping(fetchItem -> fetchItem, Collectors.toList())))))
                .entrySet())
        .flatMap(
            packageEntries ->
                Flux.fromIterable(packageEntries.getValue().entrySet())
                    .flatMap(
                        protocolEntries ->
                            Mono.just(FetchItem.buildMutationFields(protocolEntries.getValue()))
                                    ...
                                    fetchJsonValue ->
                                        Flux.fromStream(
                                            protocolEntries.getValue().stream()
                                                .filter(
                                                    fetchItem -> fetchItem.getFetchField() != null)
                                                .map(
                                                    fetchItem -> {
                                                      String alias =
                                                          Optional.ofNullable(
                                                                  fetchItem
                                                                      ...
                                                                      .getAlias())
                                                              .orElseGet(
                                                                  fetchItem.getFetchField()
                                                                      ::getName);
                                                      JsonValue fieldJsonValue =
                                                          fetchJsonValue.asJsonObject().get(alias);
                                                      return jsonProvider
                                                          .createObjectBuilder()
                                                          .add("op", "add")
                                                          .add("path", "/" + alias)
        ...
        .map(
            patchList ->
                (JsonValue)
                    jsonProvider
                        .createPatchBuilder(
                            patchList.stream().collect(JsonCollectors.toJsonArray()))
                        .build()
                                    ...
                                    arrayValueWithVariable.getValueWithVariables().stream()
                                        ...
                                        .filter(
                                            valueWithVariable ->
                                                !valueWithVariable
                                                        .asObject()
                                                        .containsKey(FIELD_DEPRECATED_NAME)
                                                    || !valueWithVariable
                                                        .asObject()
                                                        .asJsonObject()
                                                        .getBoolean(FIELD_DEPRECATED_NAME))
  ...
  public Stream<FetchItem> buildFetchItems(
      ...
      boolean merge) {
    if (jsonValue == null
        || jsonValue.getValueType().equals(JsonValue.ValueType.NULL)
    ...
    if (fieldDefinition.isFetchField()) {
      ...
      if (fieldTypeDefinition.isObject()) {
        ...
        if (fieldDefinition.hasFetchWith()) {
          ...
          FetchItem removeRelationFetchItem =
              new FetchItem(
                  packageName,
                  packageManager.isLocalPackage(fetchWithType)
          ...
          if (valueWithVariable.isNull()
              || fieldDefinition.getType().hasList() && valueWithVariable.asArray().isEmpty()) {
                            ...
                            item.asObject().containsKey(INPUT_VALUE_WHERE_NAME)
                                ...
                                ? jsonProvider
                                    ...
                                    .add(
                                        ...
                                        getFetchFrom(
                                            jsonValue.asJsonObject().get(fetchFrom),
                                            fetchWithType.getFieldOrError(fetchWithFrom)))
                                    .add(
                                        fetchWithTo,
                                        item.asObject()
                                            .getJsonObject(INPUT_VALUE_WHERE_NAME)
                                            .getJsonObject(idField.getName())
                                            .get(INPUT_OPERATOR_INPUT_VALUE_VAL_NAME))
                                    .build()
                                : jsonProvider
                                    .createObjectBuilder()
                                    .add(
                                    ...
                                    .add(
                                        fetchWithType.getFields().stream()
                                            ...
                                            .orElseThrow(
                                                () ->
                                                    new GraphQLErrors(
                                                        FETCH_WITH_TO_OBJECT_FIELD_NOT_EXIST.bind(
                                                            fetchWithTo))),
                                        item)
                                    .build())
                    .map(
                        item -> {
                          ...
                          return new FetchItem(
                              ...
                              item,
                              id,
                              fetchWithType.getIDFieldOrError().getName());
                        });
            if (merge) {
          ...
          String protocol =
              fieldDefinition
                  .getFetchProtocol()
                  .orElseGet(() -> new EnumValue(ENUM_PROTOCOL_ENUM_VALUE_LOCAL))
                  .getValue();
                      ...
                      jsonProvider
                          ...
                          .add(
                              ...
                              jsonProvider
                                  ...
                                  .add(
                                      INPUT_OPERATOR_INPUT_VALUE_VAL_NAME,
                                      jsonValue.asJsonObject().containsKey(INPUT_VALUE_WHERE_NAME)
                                              && jsonValue.asJsonObject().keySet().size() == 1
                                          ? jsonValue
                                              .asJsonObject()
                                              .getJsonObject(INPUT_VALUE_WHERE_NAME)
                                              .getJsonObject(fetchTo)
                                              .get(INPUT_OPERATOR_INPUT_VALUE_VAL_NAME)
          ...
          if (fieldDefinition.getType().hasList()) {
                        ...
                        item ->
                            jsonProvider
                                ...
                                .add(
                                    ...
                                    getFetchFrom(
                                        jsonValue.asJsonObject().get(fetchFrom),
                                        fieldTypeDefinition.asObject().getFieldOrError(fetchTo)))
                                .build())
                    .map(
                        item -> {
                          String id;
            ...
            JsonValue mutationJsonValue =
                jsonProvider
                    .createObjectBuilder(valueWithVariable.asObject())
                    .add(
                        fetchTo,
                        getFetchFrom(
                            jsonValue.asJsonObject().get(fetchFrom),
      ...
      return inputValueTypeDefinition.asInputObject().getInputValues().stream()
          .filter(
              subInputValue -> !documentManager.getInputValueTypeDefinition(subInputValue).isLeaf())
          .flatMap(
              subInputValue ->
                  Stream.ofNullable(
