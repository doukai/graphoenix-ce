...
public class FetchItem {
  ...
  public static List<Field> buildMutationFields(Collection<FetchItem> fetchItems) {
    return Stream.concat(
            fetchItems.stream().map(FetchItem::getFetchField).filter(Objects::nonNull),
            fetchItems.stream()
                .filter(fetchItem -> fetchItem.getFetchField() == null)
                .collect(Collectors.groupingBy(FetchItem::getTypeName, Collectors.toList()))
                .entrySet()
                .stream()
                .map(
                    entry ->
  ...
  public FetchItem(
      String packageName, String protocol, String path, Field fetchField, String target) {
