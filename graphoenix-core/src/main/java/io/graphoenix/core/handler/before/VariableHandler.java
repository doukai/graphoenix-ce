...
public class VariableHandler implements OperationBeforeHandler {
  ...
  public Mono<Operation> handle(Operation operation, Map<String, JsonValue> variables) {
    Map<String, JsonValue> mergedVariables =
        mergeDefaultValues(operation.getVariableDefinitions(), variables);
    return Mono.just(
        operation
            .setSelections(
                Stream.concat(
                        replaceFieldsVariables(operation.getFields(), mergedVariables)
                            .map(
                                field ->
                                    (Field)
                                        field.setDirectives(
                                            replaceDirectivesVariables(
                                                    field.getDirectives(), mergedVariables)
                                                .collect(Collectors.toList()))),
                        operation.getFragments().stream()
                            .map(
                                fragment ->
                                    (Fragment)
                                        fragment.setDirectives(
                                            replaceDirectivesVariables(
                                                    fragment.getDirectives(), mergedVariables)
                                                .collect(Collectors.toList()))))
                    .collect(Collectors.toList()))
            .setDirectives(
  ...
  public Stream<Field> replaceFieldsVariables(
      Collection<Field> fields, Map<String, JsonValue> variables) {
                        ...
                        Stream.concat(
                                replaceFieldsVariables(field.getFields(), variables)
                                    .map(
                                        subField ->
                                            (Field)
                                                subField.setDirectives(
                                                    replaceDirectivesVariables(
                                                            subField.getDirectives(), variables)
                                                        .collect(Collectors.toList()))),
                                    ...
                                    .map(
                                        fragment ->
                                            (Fragment)
                                                fragment.setDirectives(
                                                    replaceDirectivesVariables(
                                                            fragment.getDirectives(), variables)
                                                        .collect(Collectors.toList()))))
                            .collect(Collectors.toList())));
  }

  public Stream<Directive> replaceDirectivesVariables(
      Collection<Directive> directives, Map<String, JsonValue> variables) {
  ...
  public ValueWithVariable replaceVariables(
      ValueWithVariable valueWithVariable, Map<String, JsonValue> variables) {
  ...
  public Map<String, ValueWithVariable> replaceObjectVariables(
      ObjectValueWithVariable objectValueWithVariable, Map<String, JsonValue> variables) {
  ...
  public Map<String, JsonValue> mergeDefaultValues(
      Collection<VariableDefinition> variableDefinitions, Map<String, JsonValue> variables) {
    return Stream.concat(
            Stream.ofNullable(variables).map(Map::entrySet).flatMap(Collection::stream),
            Stream.ofNullable(variableDefinitions)
                .flatMap(Collection::stream)
                .filter(
                    variableDefinition ->
                        variableDefinition.getDefaultValue() != null
                            && variables.containsKey(variableDefinition.getVariable().getName()))
                .map(
                    variableDefinition ->
                        new AbstractMap.SimpleEntry<>(
