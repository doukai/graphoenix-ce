...
public class TransactionCompensator {
  ...
  public void addNewTypePath(String typeName, FetchItem fetchItem) {
    List<FetchItem> fetchItemList =
        newTypeFetchItemListMap.computeIfAbsent(typeName, k -> new ArrayList<>());
  ...
  public TransactionCompensator addTypeFetchItemListMap(
      Map<String, List<FetchItem>> newTypeFetchItemListMap) {
    this.newTypeFetchItemListMap.putAll(newTypeFetchItemListMap);
    return this;
  }

  public TransactionCompensator addRelationTypeValueListMap(
      Map<String, List<FetchItem>> newRelationTypeFetchItemListMap) {
        ...
        contextView -> {
          ...
          if (Boolean.TRUE.equals(inTransaction)) {
            ...
            Class<? extends Exception>[] dontRollbackOn =
                contextView.getOrDefault(DONT_ROLLBACK_ON, null);
            if (dontRollbackOn != null && dontRollbackOn.length > 0) {
              if (Arrays.stream(dontRollbackOn)
                  .anyMatch(exception -> exception.equals(throwable.getClass()))) {
                return Mono.empty();
              }
            } else if (rollbackOn != null && rollbackOn.length > 0) {
              if (Arrays.stream(rollbackOn)
                  .noneMatch(exception -> exception.equals(throwable.getClass()))) {
                return Mono.empty();
              }
            }
            return Mono.just(
                                    ...
                                    .map(
                                        entry -> {
                                          String id =
                                              documentManager
                                                  .getDocument()
                                                  .getObjectTypeOrError(entry.getKey())
                                                  .getIDFieldOrError()
                                                  .getName();
                                          return new Field(
                                                  typeNameToFieldName(entry.getKey()) + SUFFIX_LIST)
                                                                ...
                                                                jsonObject.get(
                                                                    Optional.ofNullable(
                                                                        ...
                                                                        .orElseGet(
                                                                            fetchItem.getField()
                                                                                ::getName));
                                                            if (pointer.containsValue(
                                                                (JsonStructure) fieldJsonValue)) {
                                                              JsonValue idValue =
                                                                  pointer.getValue(
                                                                      (JsonStructure)
                                                                          fieldJsonValue);
