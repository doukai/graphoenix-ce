...
public class MutationBeforeFetchHandler implements OperationBeforeHandler, FetchBeforeHandler {
  ...
  public static final int MUTATION_BEFORE_FETCH_HANDLER_PRIORITY =
      CONNECTION_SPLITTER_PRIORITY + 450;
  ...
  public Mono<Operation> mutation(Operation operation, Map<String, JsonValue> variables) {
    ...
    return Flux.fromIterable(
            operation.getFields().stream()
                .flatMap(
                    field -> buildFetchItems(operationType.getFieldOrError(field.getName()), field))
                .collect(
                        ...
                        protocolEntries ->
                            Mono.just(FetchItem.buildMutationFields(protocolEntries.getValue()))
                                ...
                                .flatMapMany(
                                    fetchJsonValue ->
                                        Flux.fromIterable(
                                                FetchItem.buildMutationItems(
                                                        protocolEntries.getValue())
                                                    .collect(
                                                        Collectors.groupingBy(
                                                            FetchItem::getField,
                                                            Collectors.mapping(
  ...
  public Stream<FetchItem> buildFetchItems(
      ...
      AbstractMap<String, JsonValue> arguments) {
    Definition fieldTypeDefinition = documentManager.getFieldTypeDefinition(fieldDefinition);
    if (fieldDefinition.isFetchField()) {
      if (!fieldDefinition.getType().hasList()
          && !fieldDefinition.hasFetchWith()
          && documentManager.isFetchAnchor(objectType, fieldDefinition)) {
        String protocol =
            fieldDefinition
                .getFetchProtocol()
