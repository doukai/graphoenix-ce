...
public class ConnectionBuilder implements OperationAfterHandler {
  ...
  public ConnectionBuilder(
      DocumentManager documentManager, PackageManager packageManager, JsonProvider jsonProvider) {
  ...
  public Stream<JsonValue> buildConnections(
      String path, FieldDefinition fieldDefinition, Field field, JsonValue jsonValue) {
    ...
    if (packageManager.isLocalPackage(fieldDefinition)
      ...
      if (fieldDefinition.getType().hasList()) {
                            ...
                            subField -> {
                              String subSelectionName =
                                  Optional.ofNullable(subField.getAlias())
                                      .orElse(subField.getName());
                              return buildConnections(
                                  path + "/" + index + "/" + subSelectionName,
                                  fieldTypeDefinition
                                      .asObject()
        ...
        if (fieldDefinition.isConnectionField()) {
          if (jsonValue.asJsonObject().get(selectionName + SUFFIX_LIST) == null
              || jsonValue
                  .asJsonObject()
                  .get(selectionName + SUFFIX_LIST)
                  .getValueType()
                  .equals(JsonValue.ValueType.NULL)) {
            return Stream.empty();
          }
          FieldDefinition nodeFieldDefinition =
              documentManager
                  .getFieldTypeDefinition(
                      fieldTypeDefinition.asObject().getFieldOrError(FIELD_EDGES_NAME))
                  .asObject()
                  .getFieldOrError(FIELD_NODE_NAME);
          JsonValue connectionJsonValue =
              buildConnection(
                  nodeFieldDefinition,
                  field,
                  jsonValue.asJsonObject().get(selectionName + SUFFIX_LIST),
                  jsonValue.asJsonObject().get(selectionName + SUFFIX_AGGREGATE));
                ...
                IntStream.range(0, edgesJsonValue.asJsonArray().size())
                    ...
                    .mapToObj(
                        index ->
                            Stream.ofNullable(
                                    field
                                        .getField(FIELD_EDGES_NAME)
                                        .getField(FIELD_NODE_NAME)
                                        .getFields())
                                .flatMap(Collection::stream)
                                .flatMap(
                                    subField -> {
                                      ...
                                      return buildConnections(
                                          path
                                              ...
                                              + subSelectionName,
                                          documentManager
                                              .getFieldTypeDefinition(nodeFieldDefinition)
                                              .asObject()
                                              .getFieldOrError(subField.getName()),
                                          subField,
                                          edgesJsonValue
  ...
  public JsonValue buildConnection(
      FieldDefinition nodeFieldDefinition,
      Field connectionField,
      JsonValue fieldJsonValue,
      JsonValue aggJsonValue) {
    if (connectionField.getFields() != null && !connectionField.getFields().isEmpty()) {
      ...
      ObjectType nodeFieldTypeDefinition =
          documentManager.getFieldTypeDefinition(nodeFieldDefinition).asObject();
      FieldDefinition cursorFieldDefinition =
          nodeFieldTypeDefinition
              .getCursorField()
              .orElseGet(nodeFieldTypeDefinition::getIDFieldOrError);

      int size =
          fieldJsonValue.getValueType().equals(JsonValue.ValueType.NULL)
              ? 0
              : fieldJsonValue.asJsonArray().size();
      int limit = size;
      ...
      if (connectionField.getArguments() != null && !connectionField.getArguments().isEmpty()) {
        ...
        isLast =
            connectionField.getArguments().getArguments().entrySet().stream()
                .anyMatch(entry -> entry.getKey().equals(INPUT_VALUE_LAST_NAME));
        isBefore =
            connectionField.getArguments().getArguments().entrySet().stream()
                .anyMatch(entry -> entry.getKey().equals(INPUT_VALUE_BEFORE_NAME));
        isAfter =
            connectionField.getArguments().getArguments().entrySet().stream()
                .anyMatch(entry -> entry.getKey().equals(INPUT_VALUE_AFTER_NAME));
      }
      ...
      for (Field field : connectionField.getFields()) {
        switch (field.getName()) {
          case FIELD_TOTAL_COUNT_NAME:
            ...
            connectionObjectBuilder.add(
                FIELD_TOTAL_COUNT_NAME,
                aggJsonValue.asJsonObject().get(idField.getName() + SUFFIX_COUNT));
            break;
                          ...
                          node -> {
                            ...
                            for (Field edgeField : field.getFields()) {
                              if (edgeField.getName().equals(FIELD_CURSOR_NAME)) {
                                edge.add(
                                    FIELD_CURSOR_NAME,
                                    node.asJsonObject().get(cursorFieldDefinition.getName()));
                                ...
                                edge.add(
                                    ...
                                    edgeField.getFields().stream()
                                        .map(
                                            nodeField ->
                                                Optional.ofNullable(nodeField.getAlias())
                                                    .orElseGet(nodeField::getName))
                                        .map(
                                            key ->
                                                new AbstractMap.SimpleEntry<>(
                                                    key,
                                                    node.asJsonObject().getOrDefault(key, NULL)))
              ...
              switch (pageInfoField.getName()) {
                case FIELD_HAS_NEXT_PAGE_NAME:
                  if (fieldJsonValue.getValueType().equals(JsonValue.ValueType.NULL)) {
                    ...
                    pageInfoBuilder.add(
                        FIELD_HAS_NEXT_PAGE_NAME, limit < fieldJsonValue.asJsonArray().size());
                  } else {
                ...
                case FIELD_HAS_PREVIOUS_PAGE_NAME:
                  if (fieldJsonValue.getValueType().equals(JsonValue.ValueType.NULL)) {
                    ...
                    pageInfoBuilder.add(
                        FIELD_HAS_PREVIOUS_PAGE_NAME, limit < fieldJsonValue.asJsonArray().size());
                  } else {
                ...
                case FIELD_START_CURSOR_NAME:
                  if (!fieldJsonValue.getValueType().equals(JsonValue.ValueType.NULL)
                      && !fieldJsonValue.asJsonArray().isEmpty()) {
                    if (limit < fieldJsonValue.asJsonArray().size()) {
                      if (isFirst) {
                        pageInfoBuilder.add(
                            FIELD_START_CURSOR_NAME,
                            fieldJsonValue
                                .asJsonArray()
                                .get(0)
                                .asJsonObject()
                                .get(cursorFieldDefinition.getName()));
                      } else {
                        pageInfoBuilder.add(
                            FIELD_START_CURSOR_NAME,
                            fieldJsonValue
                                .asJsonArray()
                                .get(fieldJsonValue.asJsonArray().size() - 2)
                                .asJsonObject()
                                .get(cursorFieldDefinition.getName()));
                      }
                    } else {
                      if (isFirst) {
                        ...
                        pageInfoBuilder.add(
                            ...
                            fieldJsonValue
                                .asJsonArray()
                                .get(fieldJsonValue.asJsonArray().size() - 1)
                                .asJsonObject()
                                .get(cursorFieldDefinition.getName()));
                      }
                    }
