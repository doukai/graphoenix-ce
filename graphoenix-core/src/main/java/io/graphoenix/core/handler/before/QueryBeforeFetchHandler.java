...
public class QueryBeforeFetchHandler implements OperationBeforeHandler, FetchBeforeHandler {
  ...
  @Inject
  public QueryBeforeFetchHandler(
  ...
  public Stream<FetchItem> buildFetchItems(
      String path, FieldDefinition fieldDefinition, Field field) {
    ...
    if (fieldTypeDefinition.isObject() && !fieldTypeDefinition.isContainer()) {
      return Streams.concat(
          ...
          Stream.ofNullable(fieldDefinition.getArguments())
              ...
              .filter(
                  inputValue ->
                      inputValue.getType().getTypeName().getName().endsWith(SUFFIX_EXPRESSION))
              .flatMap(
                  inputValue ->
                      Stream.ofNullable(
                  ...
                  exsInputValue ->
                      Stream.ofNullable(field.getArguments())
                          .flatMap(
                              arguments ->
                                  arguments
                                      .getArgumentOrEmpty(exsInputValue.getName())
                                      .or(
                                          () ->
                                              Optional.ofNullable(exsInputValue.getDefaultValue()))
                                      .stream())
                              ...
                              arrayValueWithVariable ->
                                  IntStream.range(0, arrayValueWithVariable.size())
                                      .mapToObj(
                                          index ->
                                              documentManager
                                                  .getInputValueTypeDefinition(exsInputValue)
                                                  .asInputObject()
                                                  .getInputValues()
                                                  .stream()
                                                  .flatMap(
                  ...
                  whereInputValue ->
                      Stream.ofNullable(field.getArguments())
                          .flatMap(
                              arguments ->
                                  arguments
                                      .getArgumentOrEmpty(whereInputValue.getName())
                                      .or(
                                          () ->
                                              Optional.ofNullable(
                                                  whereInputValue.getDefaultValue()))
                                      .stream())
                          ...
                          .flatMap(
                              objectValueWithVariable ->
                                  documentManager
                                      .getInputValueTypeDefinition(whereInputValue)
                                      .asInputObject()
                                      .getInputValues()
                                      .stream()
                                      .filter(
    ...
    if (fieldDefinition.isFetchField()) {
                ...
                Map.of(
                    fetchWithType.getFields().stream()
                        ...
                        .orElseThrow(
                            () ->
                                new GraphQLErrors(
                                    FETCH_WITH_TO_OBJECT_FIELD_NOT_EXIST.bind(fetchWithTo))),
                    valueWithVariable,
      ...
      return Streams.concat(
          inputValueTypeDefinition.asInputObject().getInputValues().stream()
              .filter(
                  subInputValue ->
                      subInputValue.getType().getTypeName().getName().endsWith(SUFFIX_EXPRESSION))
              .flatMap(
                  subInputValue ->
                      Stream.ofNullable(
          ...
          inputValueTypeDefinition
              .asInputObject()
              .getInputValueOrEmpty(INPUT_VALUE_EXS_NAME)
              .stream()
              .flatMap(
                  exsInputValue ->
                      valueWithVariable
                          .asObject()
                          .getValueWithVariableOrEmpty(exsInputValue.getName())
                          .or(() -> Optional.ofNullable(exsInputValue.getDefaultValue()))
                          .stream()
                          .filter(ValueWithVariable::isArray)
                          .map(ValueWithVariable::asArray)
                          .flatMap(
          ...
          inputValueTypeDefinition
              .asInputObject()
              .getInputValueOrEmpty(INPUT_VALUE_WHERE_NAME)
              .stream()
              .flatMap(
                  whereInputValue ->
                      valueWithVariable
                          .asObject()
                          .getValueWithVariableOrEmpty(whereInputValue.getName())
                          .or(() -> Optional.ofNullable(whereInputValue.getDefaultValue()))
                          .stream()
                          .filter(ValueWithVariable::isObject)
                          .map(ValueWithVariable::asObject)
                          .flatMap(
