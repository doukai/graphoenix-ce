...
public class UniqueMergeHandler implements OperationBeforeHandler {
  ...
  public Mono<Operation> mutation(Operation operation, Map<String, JsonValue> variables) {
    ...
    return Flux.fromStream(
            operation.getFields().stream()
                ...
                .flatMap(
                    field ->
                        buildUniqueItems(operationType.getFieldOrError(field.getName()), field))
                ...
                .flatMap(
                    typeEntry ->
                        typeEntry.getValue().stream()
                            .collect(Collectors.groupingBy(Tuple5::getT2, Collectors.toList()))
                            .entrySet()
                            .stream()
                            .map(
                                fieldEntry ->
                                    new AbstractMap.SimpleEntry<>(
                                        new Field(
                                                typeNameToFieldName(typeEntry.getKey().getName())
                                                    + SUFFIX_LIST)
                                            .setAlias(
                                                typeNameToFieldName(typeEntry.getKey().getName())
                                                    + "_"
  ...
  public Stream<Tuple5<ObjectType, String, ValueWithVariable, Field, String>> buildUniqueItems(
                          ...
                          .flatMap(
                              subFieldDefinition ->
                                  Stream.ofNullable(field.getArguments())
                                      .filter(
                                          arguments ->
                                              !hasIDValueWithVariable(
                                                  idField.getName(), field.getArguments()))
                                      .flatMap(
                                          arguments ->
                                              arguments
  ...
  public Stream<Tuple5<ObjectType, String, ValueWithVariable, Field, String>> buildUniqueItems(
    ...
    if (fieldDefinition.isUnique()) {
      return Stream.of(
          Tuples.of(objectType, fieldDefinition.getName(), valueWithVariable, field, path));
                          ...
                          subFieldDefinition -> {
                            if (fieldDefinition.getType().hasList()) {
                              ...
                              return Stream.ofNullable(valueWithVariable.asObject())
                                  .filter(
                                      objectValue ->
                                          !hasIDValueWithVariable(idField.getName(), objectValue))
                                  .flatMap(
                                      objectValue ->
                                          Optional.ofNullable(
                                              objectValue.getValueWithVariable(
                                                  subInputValue.getName()))
                                              .or(
                                                  () ->
                                                      Optional.ofNullable(
  ...
  private boolean hasIDValueWithVariable(
