...
public class PackageOperationDAO implements Asyncable {
  ...
  private Mono<JsonValue> fetchAsync(
      String packageName, String protocol, String graphql, Map<String, Object> parameters) {
                    ...
                    .map(
                        entry ->
                            new AbstractMap.SimpleEntry<>(
                                entry.getKey(), ValueWithVariable.of(entry.getValue())))
                    .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue))
                : null)
        .flatMap(
            operation ->
                CDI.current()
                    .select(PackageFetchHandler.class, NamedLiteral.of(protocol))
                    .get()
