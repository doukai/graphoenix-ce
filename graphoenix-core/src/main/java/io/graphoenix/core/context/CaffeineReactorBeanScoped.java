...
public abstract class CaffeineReactorBeanScoped implements ReactorBeanScoped {
  ...
  public Mono<String> getScopedKey() {
    return Mono.deferContextual(
        contextView -> Mono.justOrEmpty(contextView.getOrEmpty(getScopedKeyName())));
  }
    ...
    return getScopedKey()
        .flatMap(
            key -> {
              ...
              return supplier
                  .get()
                  .map(bean -> (T) scopedMap.compute(beanClass.getName(), (k, v) -> bean));
            });
  }
  ...
  public <T, R extends T> Mono<Boolean> put(Class<T> beanClass, R bean) {
    return getScopedKey()
        .doOnSuccess(
            key ->
                getCache().get(key, k -> new ConcurrentHashMap<>()).put(beanClass.getName(), bean))
