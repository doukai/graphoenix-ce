...
public class ArgumentsToFilter {
  ...
  public Optional<Expression> argumentsToMultipleExpression(
      FieldDefinition fieldDefinition, Field field) {
    String path = "@";
                                  ...
                                  .flatMap(
                                      subField -> {
                                        if (subField.getType().hasList()) {
                                          return inputValueToMultipleExpression(
                                              subField, entry.getKey(), entry.getValue(), "@")
                                              .map(
                                                  expression ->
                                                      new NotEqualsTo(
                                                          new Filter(
                                                              path + "." + subField.getName(),
                                                              expression),
                                                          new EmptyArray()))
                                              .stream();
                                        } else {
                      ...
                      .flatMap(
                          entry ->
                              Stream.of(entry.getValue())
                                  ...
                                  .flatMap(
                                      valueWithVariable ->
                                          inputValueToMultipleExpression(
                                              fieldDefinition,
                                              entry.getKey(),
                                              valueWithVariable,
                                              path,
                          ...
                          .anyMatch(
                              entry ->
                                  entry
                                          .getKey()
                                          .getName()
                                          .equals(INPUT_VALUE_INCLUDE_DEPRECATED_NAME)
                                      && entry.getValue().isBoolean()
                                      && entry.getValue().asBoolean().getValue())
                      ? Stream.empty()
                      : Stream.of(
                          new NotEqualsTo(
                              path + "." + FIELD_DEPRECATED_NAME, new BooleanValue(true))))
              .collect(Collectors.toList());

      return expressionListToMultipleExpression(
          expressionList, isOr(field.getArguments()), isNot(field.getArguments()), true);
    } else {
              ...
              opr ->
                  Optional.of(field.getArguments())
                      .flatMap(
                          arguments ->
                              arguments
                                  .getArgumentOrEmpty(INPUT_OPERATOR_INPUT_VALUE_VAL_NAME)
                                  .flatMap(
                                      val ->
  ...
  protected Optional<Expression> inputValueToMultipleExpression(
      FieldDefinition fieldDefinition,
      InputValue inputValue,
      ValueWithVariable valueWithVariable,
      String path) {
    return inputValueToMultipleExpression(
        fieldDefinition, inputValue, valueWithVariable, path, false);
  }
                          ...
                          entry ->
                              Stream.of(entry.getValue())
                                  ...
                                  .flatMap(
                                      field ->
                                          inputValueToMultipleExpression(
                                              fieldDefinition, entry.getKey(), field, path, true)
                                              .stream())),
                  exs
                          || inputValueValueWithVariableMap.entrySet().stream()
                              .anyMatch(
                          ...
                          objectValueWithVariable ->
                              objectValueWithVariable
                                  ...
                                  .flatMap(
                                      val ->
                                          operatorToExpression(
                                              path, opr, val, skipNull(objectValueWithVariable)))
                                  .or(
                                      () ->
                                          objectValueWithVariable
                                              .getValueWithVariableOrEmpty(
  ...
  protected Optional<Expression> expressionListToMultipleExpression(
      List<Expression> expressionList, boolean isOr, boolean isNot, boolean isRoot) {
  ...
  private Optional<Expression> operatorToExpression(
      String path, String opr, ValueWithVariable val, boolean skipNull) {
                            ...
                            return new MultiAndExpression(
                                Arrays.asList(
                                    new GreaterThanEquals(
                                        path, valueToJsonValue(valueWithVariables.get(0))),
                                    new MinorThanEquals(
                                        path, valueToJsonValue(valueWithVariables.get(1)))));
                          } else {
