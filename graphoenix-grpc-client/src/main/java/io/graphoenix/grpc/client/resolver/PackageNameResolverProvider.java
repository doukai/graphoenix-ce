      ...
      private void resolve() {
        try {
          List<EquivalentAddressGroup> addresses =
              packageManager
                  .getURLList(targetUri.getHost(), Hammurabi.ENUM_PROTOCOL_ENUM_VALUE_GRPC)
                  .stream()
                  .map(
                      packageURL ->
                          new InetSocketAddress(packageURL.getHost(), packageURL.getPort()))
                  .map(EquivalentAddressGroup::new)
                  .collect(Collectors.toList());

          ResolutionResult resolutionResult =
              ResolutionResult.newBuilder().setAddresses(addresses).build();
          ...
          this.listener.onError(
              Status.UNAVAILABLE.withDescription("Unable to resolve host ").withCause(e));
        }
      }
    };
  }
