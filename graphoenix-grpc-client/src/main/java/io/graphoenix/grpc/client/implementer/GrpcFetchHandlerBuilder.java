...
public class GrpcFetchHandlerBuilder {
  ...
  @Inject
  public GrpcFetchHandlerBuilder(
  ...
  private JavaFile buildClass() {
    return JavaFile.builder(packageConfig.getGrpcHandlerPackageName(), buildGrpcFetchHandler())
        .build();
  }

  private TypeSpec buildGrpcFetchHandler() {
    TypeSpec.Builder builder =
        TypeSpec.classBuilder("GrpcPackageFetchHandler")
            ...
            .addAnnotation(
                AnnotationSpec.builder(Named.class)
                    .addMember(
                        "value", "$T.ENUM_PROTOCOL_ENUM_VALUE_GRPC", ClassName.get(Hammurabi.class))
                    .build())
            .addField(
                FieldSpec.builder(
    ...
    packageNameSet.forEach(
        packageName ->
            builder.addField(
                FieldSpec.builder(
                        ClassName.get(
                            packageName + ".grpc", "GraphQLServiceGrpc", "GraphQLServiceStub"),
  ...
  private MethodSpec buildConstructor() {
    MethodSpec.Builder builder =
        MethodSpec.constructorBuilder()
            ...
            .addParameter(
                ClassName.get(PackageNameResolverProvider.class), "packageNameResolverProvider")
            .addStatement("this.jsonProvider = jsonProvider")
            .addStatement(
                "$T.getDefaultRegistry().register(packageNameResolverProvider)",
                ClassName.get(NameResolverRegistry.class));

    packageNameSet.forEach(
        packageName ->
            builder.addStatement(
                "this.$L = $T.newStub($T.forTarget($S).defaultLoadBalancingPolicy($S).usePlaintext().build())",
                packageNameToUnderline(packageName) + "_GraphQLServiceStub",
                ClassName.get(packageName + ".grpc", "GraphQLServiceGrpc"),
                ClassName.get(ManagedChannelBuilder.class),
                "package://" + packageName,
                CaseFormat.LOWER_CAMEL.to(
                    CaseFormat.LOWER_UNDERSCORE, packageConfig.getPackageLoadBalance())));
    return builder.build();
  }

  private MethodSpec buildOperationMethod() {
    MethodSpec.Builder builder =
        MethodSpec.methodBuilder("request")
            .addModifiers(Modifier.PUBLIC)
            ...
            .addParameter(
                ParameterSpec.builder(ClassName.get(Operation.class), "operation").build())
        ...
        packageName ->
            builder.addStatement(
                CodeBlock.builder()
                    ...
                    .indent()
                    .add(
                        "return $T.<$T>create(fluxSink ->\n",
                        ClassName.get(Flux.class),
                        ClassName.get(packageName + ".grpc", "GraphQLResponse"))
                    .indent()
                    .add(
                        "this.$L.request($T.newBuilder().setRequest(operation.toString()).build(), new $T<$T>() {\n",
                        packageNameToUnderline(packageName) + "_GraphQLServiceStub",
                    ...
                    .add(
                        "public void onNext($T value) { fluxSink.next(value); }\n",
                        ClassName.get(packageName + ".grpc", "GraphQLResponse"))
                    .add("@$T\n", ClassName.get(Override.class))
                    .add(
                        "public void onError($T t) { fluxSink.error(t); }\n",
                    ...
                    .add(".last()\n")
                    .add(
                        ".map(response -> jsonProvider.createReader(new $T(response.getResponse())).readValue())",
