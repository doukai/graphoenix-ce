...
public class ArgumentsInvokeHandlerBuilder {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(ArgumentsInvokeHandlerBuilder.class.getName());

  private final DocumentManager documentManager;
  private final PackageConfig packageConfig;

  @Inject
  public ArgumentsInvokeHandlerBuilder(
      DocumentManager documentManager, PackageConfig packageConfig) {
    ...
    return TypeSpec.classBuilder("ArgumentsInvokeHandler")
        ...
        .addAnnotation(
            AnnotationSpec.builder(Priority.class)
                .addMember(
                    "value",
                    "$T.FRAGMENT_HANDLER_PRIORITY + 100",
                    ClassName.get(FragmentHandler.class))
                .build())
        .addField(
            FieldSpec.builder(
  ...
  private MethodSpec buildTypeInvokeMethod(ObjectType objectType) {
    ...
    if (isOperationType) {
      builder
          .returns(
              ParameterizedTypeName.get(ClassName.get(Mono.class), ClassName.get(Operation.class)))
          .addAnnotation(Override.class)
          .addParameter(ClassName.get(Operation.class), "operation")
          .addParameter(
              ParameterizedTypeName.get(Map.class, String.class, JsonValue.class), "variables");
    } else {
      builder
