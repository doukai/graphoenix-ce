...
public class InputInvokeHandlerBuilder {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(InputInvokeHandlerBuilder.class.getName());
  ...
  @Inject
  public InputInvokeHandlerBuilder(
      DocumentManager documentManager, PackageManager packageManager, PackageConfig packageConfig) {
  ...
  public void writeToFiler(Filer filer) throws IOException {
    this.directiveInvokeClassSet =
        documentManager
            ...
            .flatMap(
                fieldDefinition -> documentManager.getDirectiveInvokes(fieldDefinition).stream())
            .filter(
                objectValueWithVariable ->
                    packageManager.isLocalPackage(
                        objectValueWithVariable.getString(
                            INPUT_INVOKE_INPUT_VALUE_PACKAGE_NAME_NAME)))
            .filter(
                objectValueWithVariable ->
                    objectValueWithVariable.getBoolean(
                            INPUT_INVOKE_INPUT_VALUE_ON_EXPRESSION_NAME, false)
                        || objectValueWithVariable.getBoolean(
                            INPUT_INVOKE_INPUT_VALUE_ON_INPUT_VALUE_NAME, false))
            .map(
                objectValueWithVariable ->
                    objectValueWithVariable.getString(INPUT_INVOKE_INPUT_VALUE_CLASS_NAME_NAME))
  ...
  private Stream<JavaFile> buildClasses() {
    return Stream.of(
        JavaFile.builder(packageConfig.getHandlerPackageName(), buildQueryArgumentsInvokeHandler())
            .build(),
        JavaFile.builder(
                packageConfig.getHandlerPackageName(), buildListQueryArgumentsInvokeHandler())
            .build(),
        JavaFile.builder(
                packageConfig.getHandlerPackageName(), buildConnectionQueryArgumentsInvokeHandler())
            .build(),
        JavaFile.builder(
                packageConfig.getHandlerPackageName(), buildMutationArgumentsInvokeHandler())
            .build(),
        JavaFile.builder(
                packageConfig.getHandlerPackageName(), buildListMutationArgumentsInvokeHandler())
            .build(),
            ...
            .filter(
                inputObjectType ->
                    inputObjectType.getName().endsWith(TYPE_QUERY_NAME + SUFFIX_ARGUMENTS)
                        && !inputObjectType
                            .getName()
                            .endsWith(SUFFIX_LIST + TYPE_QUERY_NAME + SUFFIX_ARGUMENTS)
                        && !inputObjectType
                            .getName()
                            .endsWith(SUFFIX_CONNECTION + TYPE_QUERY_NAME + SUFFIX_ARGUMENTS))
            .filter(
                inputObjectType -> documentManager.getInputObjectBelong(inputObjectType) != null)
    ...
    return TypeSpec.classBuilder("QueryArgumentsInvokeHandler")
        ...
        .addField(
            FieldSpec.builder(
                ...
                .build())
        .addField(
            FieldSpec.builder(ClassName.get(Jsonb.class), "jsonb", Modifier.PRIVATE, Modifier.FINAL)
                .build())
        .addField(
            FieldSpec.builder(
            ...
            .filter(
                inputObjectType ->
                    inputObjectType.getName().endsWith(TYPE_MUTATION_NAME + SUFFIX_ARGUMENTS)
                        && !inputObjectType
                            .getName()
                            .endsWith(SUFFIX_LIST + TYPE_MUTATION_NAME + SUFFIX_ARGUMENTS))
            .filter(
                inputObjectType -> documentManager.getInputObjectBelong(inputObjectType) != null)
            .collect(Collectors.toList());
    ...
    return Stream.concat(
            inputObjectTypes.stream()
                ...
                .map(
                    objectValueWithVariable ->
                        objectValueWithVariable.getString(
                            INPUT_INVOKE_INPUT_VALUE_CLASS_NAME_NAME)),
                        ...
                        className ->
                            ParameterSpec.builder(
                                    ParameterizedTypeName.get(
                                        ClassName.get(Provider.class), className),
                                    typeNameToFieldName(className.simpleName()))
                                .build())
                    .collect(Collectors.toSet()))
  ...
  private MethodSpec buildInputTypeInvokeMethod(
      InputObjectType inputObjectType, boolean arguments) {
    ...
    MethodSpec.Builder builder =
        MethodSpec.methodBuilder(typeParameterName)
            .addModifiers(Modifier.PUBLIC)
    ...
    List<ObjectValueWithVariable> invokes =
        getInvokes(inputObjectType).collect(Collectors.toList());
    if (!invokes.isEmpty()) {
      int index = 0;
      for (ObjectValueWithVariable invoke : invokes) {
        String apiVariableName =
            typeNameToFieldName(
                toClassName(invoke.getString(INPUT_INVOKE_INPUT_VALUE_CLASS_NAME_NAME))
                    .simpleName());
        String methodName = invoke.getString(INPUT_INVOKE_INPUT_VALUE_METHOD_NAME_NAME);
        ClassName returnClassName =
            toClassName(
                getClassName(invoke.getString(INPUT_INVOKE_INPUT_VALUE_RETURN_CLASS_NAME_NAME)));
        ...
        if (index == 0) {
          codeBlockBuilder.add(
              "return $L.map(result -> (($T)result))\n", invokeCodeBlock, typeClassName);
        } else {
          codeBlockBuilder.add(
              ".flatMap(result -> $L).map(result -> (($T)result))\n",
              invokeCodeBlock,
              typeClassName);
        }
        index++;
    ...
    inputObjectType.getInputValues().stream()
        .filter(
            inputValue ->
                documentManager.getInputValueTypeDefinition(inputValue).isLeaf()
                    || documentManager.getInputBelong(
                                documentManager.getInputValueTypeDefinition(inputValue))
                            != null
                        && documentManager
                            .getInputBelong(documentManager.getInputValueTypeDefinition(inputValue))
                            .isLeaf())
        .filter(inputValue -> objectType.getField(inputValue.getName()) != null)
        .filter(
            inputValue ->
                !documentManager
                    .getDirectiveInvokes(objectType.getField(inputValue.getName()))
                    .isEmpty())
        .filter(
            inputValue ->
                documentManager
        ...
        .forEach(
            inputValue -> {
              ...
              CodeBlock getFieldCodeBlock =
                  CodeBlock.of(
                      "$L.$L()",
                      resultParameterName,
                      getFieldGetterMethodName(fieldDefinition.getName()));
              CodeBlock invokesCodeBlock =
                  documentManager.getDirectiveInvokes(fieldDefinition).stream()
                      ...
                      .reduce(
                          ...
                          (codeBlock, objectValueWithVariable) -> {
                            ...
                            String methodName =
                                objectValueWithVariable.getString(
                                    INPUT_INVOKE_INPUT_VALUE_METHOD_NAME_NAME);
                            ClassName returnClassName =
                                toClassName(
                                    getClassName(
                                        objectValueWithVariable.getString(
                                            INPUT_INVOKE_INPUT_VALUE_RETURN_CLASS_NAME_NAME)));
                            boolean async =
                                objectValueWithVariable.getBoolean(
                                    INPUT_INVOKE_INPUT_VALUE_ASYNC_NAME, false);
                            int parametersCount =
                                objectValueWithVariable
                                    .getJsonArray(INPUT_INVOKE_INPUT_VALUE_PARAMETER_NAME)
                                    .size();
                            String directiveName =
                                objectValueWithVariable.getString(
                                    INPUT_INVOKE_INPUT_VALUE_DIRECTIVE_NAME_NAME);
                            CodeBlock invokeCodeBlock;
                            if (parametersCount == 1) {
                              if (async) {
                                invokeCodeBlock =
                                  ...
                                  .canonicalName()
                                  .equals(Mono.class.getCanonicalName())) {
                                invokeCodeBlock =
                                    CodeBlock.of(
                                        "$L.get().$L($L)",
                                        apiVariableName,
                                  ...
                                  .canonicalName()
                                  .equals(Flux.class.getCanonicalName())) {
                                invokeCodeBlock =
                                    CodeBlock.of(
                                        "$L.get().$L($L).last()",
                                        apiVariableName,
                    ...
                    CodeBlock.join(
                        Streams.concat(
                                inputObjectType.getInputValues().stream()
                                    ...
                                    .filter(
                                        inputValue ->
                                            documentManager.getInputObjectBelong(
                                                    documentManager
                                                        .getInputValueTypeDefinition(inputValue)
                                                        .asInputObject())
                                                != null)
                                    ...
                                    .map(
                                        inputValue -> {
                                          InputObjectType inputValueType =
                                              documentManager
                                                  .getInputValueTypeDefinition(inputValue)
                                                  .asInputObject();
                                          String methodName =
                                          ...
                                          CodeBlock caseCodeBlock =
                                              CodeBlock.of("case $S:\n", inputValue.getName());
                                          CodeBlock invokeCodeBlock =
                                              CodeBlock.of(
                                                  "return $T.justOrEmpty($L.$L()).flatMap(field -> $L(field, entry.getValue().asObject())).doOnNext($L -> $L.$L($L));",
                                                  ClassName.get(Mono.class),
                                                  resultParameterName,
            ...
            inputObjectType.getInterfaces().stream()
                .flatMap(
                    interfaceObjectName ->
                        documentManager
                            .getDocument()
                            .getInputObjectType(interfaceObjectName)
                            .stream())
                .flatMap(this::getInvokes))
        .filter(
            objectValueWithVariable ->
                packageManager.isLocalPackage(
                    objectValueWithVariable.getString(INPUT_INVOKE_INPUT_VALUE_PACKAGE_NAME_NAME)))
        .filter(
